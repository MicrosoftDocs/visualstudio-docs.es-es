...
# <a name="how-to-remove-managed-code-extensions-from-documents"></a>Procedimiento Quitar extensiones de código administrado de documentos
 ...
 Puede quitar el ensamblado de personalización mediante uno de los `RemoveCustomization` métodos proporcionados [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)]por. El método que use dependerá de si desea quitar la personalización en tiempo de ejecución (es decir, ejecutando código en la personalización mientras el documento de Word o el libro de Excel está abierto) o si desea quitar la personalización de un documento cerrado o de un documento que s en un servidor que no tiene Microsoft Office instalado.

 ![vínculo a vídeo](../vsto/media/playvideo.gif "vínculo a vídeo") Para ver una demostración en vídeo relacionada, consulte [cómo: ¿Adjuntar o desasociar un ensamblado de VSTO de un documento de Word? ](http://go.microsoft.com/fwlink/?LinkId=136782).
...
1. En el código de personalización, llame <xref:Microsoft.Office.Tools.Word.Document.RemoveCustomization%2A> al método (para Word) o <xref:Microsoft.Office.Tools.Excel.Workbook.RemoveCustomization%2A> al método (para Excel). Solo se debe llamar a este método después de que ya no se necesite la personalización.

     La llamada a este método en el código depende de cómo se use la personalización. Por ejemplo, si los clientes usan las características de la personalización hasta que estén listas para enviar el documento a otros clientes que solo necesiten el propio documento (no la personalización), puede proporcionar una interfaz de `RemoveCustomization` usuario que llama cuando el cliente hace clic en él. Como alternativa, si la personalización rellena el documento con datos cuando se abre por primera vez, pero la personalización no proporciona ninguna otra característica a la que los clientes acceden directamente, puede llamar a RemoveCustomization tan pronto como la personalización. finaliza la inicialización del documento.
...
3. Llame al método <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument.RemoveCustomization%2A> estático de la <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument> clase y especifique la ruta de acceso del documento de la solución para el parámetro.
