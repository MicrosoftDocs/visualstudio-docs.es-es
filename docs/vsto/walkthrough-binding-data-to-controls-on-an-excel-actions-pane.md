...
1. Cree un proyecto de libro de Excel con el nombre **mi panel de acciones de Excel**. En el asistente, seleccione **crear un nuevo documento**. Para obtener más información, vea [Cómo: Cree proyectos de Office en Visual](../vsto/how-to-create-office-projects-in-visual-studio.md)Studio.
...
## <a name="add-controls-to-the-worksheet"></a>Agregar controles a la hoja de cálculo
 A continuación, agregue <xref:Microsoft.Office.Tools.Excel.NamedRange> un control y <xref:Microsoft.Office.Tools.Excel.ListObject> un control a la primera hoja de cálculo.

### <a name="to-add-a-namedrange-control-and-a-listobject-control"></a>Para agregar un control NamedRange y un control ListObject

1. Compruebe que el libro **mis acciones de Excel panel. xlsx** está abierto en el diseñador de Visual Studio `Sheet1` , con la muestra.
...
4. Arrastre **Company Name** desde la ventana **orígenes de datos** hasta la celda `Sheet1` **a2** en.

     Se <xref:Microsoft.Office.Tools.Excel.NamedRange> crea un `CompanyNameNamedRange` control denominado y el texto \<CompanyName > aparece en la celda **a2**. Al mismo tiempo, se agregan `suppliersBindingSource`al proyecto un <xref:System.Windows.Forms.BindingSource> denominado, un adaptador <xref:System.Data.DataSet> de tabla y un. El control está enlazado a <xref:System.Windows.Forms.BindingSource>, que a su vez se enlaza a <xref:System.Data.DataSet> la instancia de.
...
6. Haga clic en **ListObject** en la lista desplegable y, a continuación, arrastre la tabla **Products** hasta la celda **A6** en `Sheet1`.
     ...
     Se <xref:Microsoft.Office.Tools.Excel.ListObject> crea un `ProductNameListObject` control denominado en la celda **A6**. Al mismo tiempo, se agregan `productsBindingSource` al proyecto un <xref:System.Windows.Forms.BindingSource> nombre y un adaptador de tabla. El control está enlazado a <xref:System.Windows.Forms.BindingSource>, que a su vez se enlaza a <xref:System.Data.DataSet> la instancia de.

7. Solo C# para, seleccione **suppliersBindingSource** en la bandeja de componentes y cambie la propiedad **Modificadores** a **interno** en la ventana **propiedades** .
...
1. Desde las pestañas **controles comunes** del **cuadro de herramientas**, <xref:System.Windows.Forms.ComboBox> arrastre un control hasta el control panel de acciones.
...
3. En C#, debe crear un controlador de eventos para `ActionsControl`. Este código se puede colocar en el `ActionsControl` constructor. Para obtener más información acerca de la creación de controladores [de eventos, consulte Cómo: Cree controladores de eventos en proyectos](../vsto/how-to-create-event-handlers-in-office-projects.md)de Office.
...
3. En el <xref:Microsoft.Office.Tools.Excel.Workbook.Startup> controlador de eventos `ThisWorkbook`de, agregue el control al panel de acciones.
...
3. Seleccione una compañía en el cuadro de lista. Compruebe que el nombre de la empresa aparece en <xref:Microsoft.Office.Tools.Excel.NamedRange> el control y que los detalles del producto se muestran <xref:Microsoft.Office.Tools.Excel.ListObject> en el control.
...
- Enlazar datos a controles de Word. Para obtener más información, vea [Tutorial: Enlazar datos a controles en un panel](../vsto/walkthrough-binding-data-to-controls-on-a-word-actions-pane.md)de acciones de Word.
