---
title: Implementación de una solución de Office mediante ClickOnce
...
ms.locfileid: "107828623"
---
# <a name="deploy-an-office-solution-by-using-clickonce"></a>Implementación de una solución de Office mediante ClickOnce
  Puede implementar su solución de Office en menos pasos si usa ClickOnce. Si publica actualizaciones, la solución las detectará e instalará automáticamente. Sin embargo, ClickOnce requiere que instale la solución por separado para cada usuario de un equipo. Por lo tanto, debe considerar la posibilidad Windows Installer (*.msi*) si más de un usuario ejecutará la solución en el mismo equipo.
...
- [Revertir una solución a una versión anterior](#Roll)
  ...
  Para obtener más información sobre cómo implementar una solución de Office mediante la creación de un archivo Windows Installer, vea Implementar una solución de [Office mediante Windows Installer](../vsto/deploying-a-vsto-solution-by-using-windows-installer.md).

## <a name="publish-the-solution"></a><a name="Publish"></a> Publicación de la solución
...
3. En el **Diseñador de proyectos**, elija la **pestaña** Publicar, que se muestra en la ilustración siguiente.

    ![Pestaña Publicar del Diseñador de proyectos](../vsto/media/vsto-publishtab.png "Pestaña Publicar del Diseñador de proyectos")

4. En el cuadro Ubicación de la carpeta de publicación (servidor FTP o ruta  de acceso **de archivo),** escriba la ruta de acceso de la carpeta en la que desea que el Diseñador de proyectos copie los archivos de la solución.
   ...
   - Ruta de acceso de convención de nomenclatura uniforme (UNC) a una carpeta de la red (por ejemplo, *\\ \ServerName\FolderName).*

   - Ruta de acceso relativa (por ejemplo, *PublishFolder \\*, que es la carpeta en la que se publica el proyecto de forma predeterminada).

5. En el **cuadro Dirección URL de la** carpeta de instalación, escriba la ruta de acceso completa de la ubicación en la que los usuarios finales encontrarán la solución.
...
7. En el cuadro **de diálogo Requisitos previos,** asegúrese de que la casilla Crear programa de instalación **para instalar** componentes de requisitos previos está activada.

8. En la **lista Elegir qué requisitos previos** instalar, active las casillas **de Windows Installer 4.5** y el paquete de .NET Framework adecuado.

    Por ejemplo, si la solución tiene como destino , active las casillas [!INCLUDE[net_v45](../vsto/includes/net-v45-md.md)] **para Windows Installer 4.5** y **Microsoft .NET Framework 4.5 Full**.

9. Si la solución tiene como destino .NET Framework versión 4.5, active también **Visual Studio 2010 Tools for Office Runtime** (Herramientas de 2010 para Office Runtime).

    > [!NOTE]
    > De forma predeterminada, esta casilla no aparece. Para mostrar esta casilla, debe crear un paquete de programa previo. Vea Crear un paquete de programa previo para un complemento vsTO de [Office 2013 con Visual Studio 2012.](create-vsto-add-ins-for-office-by-using-visual-studio.md)
...
11. Elija el **botón** Actualizaciones, especifique la frecuencia con la que desea que el complemento o la personalización de VSTO de cada usuario final compruebe si hay actualizaciones y, a continuación, elija **el botón** Aceptar.

    > [!NOTE]
    > Si va a realizar la implementación mediante un CD o una unidad extraíble, elija el botón de opción **No buscar** actualizaciones.
...
13. Elija el **botón Publicar** ahora.
    ...
    - Carpeta **Archivos de** aplicación.
    ...
    - Ensamblados de personalización
      ...
      ![Estructura de carpetas de publicación](../vsto/media/publishfolderstructure.png "Estructura de carpetas de la carpeta de publicación")
...
## <a name="decide-how-you-want-to-grant-trust-to-the-solution"></a><a name="Trust"></a> Decidir cómo quiere conceder confianza a la solución
...
### <a name="install-the-solution-by-opening-the-deployment-manifest-from-an-iis-website"></a>Para instalar la solución, abra el manifiesto de implementación desde un sitio web de IIS.
...
1. En el servidor que ejecuta IIS 6.0, elija **Iniciar** todos los programas Herramientas  >    >    >   **administrativas Internet Information Services (IIS) Manager**.

2. Elija el nombre del equipo, **la carpeta Sitios** web o el sitio web que va a configurar.
...
6. En la **ventana Tipo MIME,** escriba **.vsto** como extensión, escriba **application/x-ms-vsto** como tipo MIME y, a continuación, aplique la nueva configuración.
    ...
    > [!NOTE]
    > Para que los cambios surtan efecto, debe reiniciar el Servicio de publicación World Wide Web o esperar hasta que el proceso de trabajo se recicle. A continuación, debe vaciar la caché de disco del explorador e intentar abrir de nuevo *el archivo .vsto.*
...
2. Abra el menú contextual del símbolo **del sistema** y elija Ejecutar  **como administrador.**

3. En el **cuadro** Abrir, escriba la ruta de acceso siguiente y, a continuación, elija **el botón** Aceptar.
...
4. Escriba el comando siguiente y, a continuación, aplique la nueva configuración.
    ...
    > [!NOTE]
    > Para que los cambios surtan efecto, debe reiniciar el Servicio de publicación World Wide Web o debe esperar hasta que el proceso de trabajo se recicle. A continuación, debe vaciar la memoria caché del disco del explorador e intentar abrir de nuevo *el archivo .vsto.*
...
4. En el **cuadro de diálogo** Agregar referencia, elija la pestaña **Proyectos,** **ArchivoCopyPDA** y, a continuación, haga clic **en el botón** Aceptar.

5. En **Explorador de soluciones**, elija el **proyecto ExcelWorkbook.**

6. En la barra de menús, **elija Project** New  >  **Folder (Proyecto nueva carpeta).**

7. Escriba **Datos** y, a continuación, elija **la tecla** Entrar.

8. En **Explorador de soluciones**, elija la **carpeta** Datos.

9. En la barra de menús, elija **Proyecto Agregar**  >  **elemento existente.**

10. En el **cuadro de diálogo Agregar** elemento existente, vaya al directorio de salida del proyecto **ExcelWorkbook,** elija el archivoExcelWorkbook.xlsxy, **a** continuación, elija **el botón** Agregar.

11. En **Explorador de soluciones** elija el **ExcelWorkbook.xlsx** archivo.

12. En la ventana **Propiedades,** cambie la propiedad  **Acción** de compilación a **Contenido** y la propiedad Copiar en directorio de salida **a Copiar si es más reciente.**

     Cuando haya completado estos pasos, el proyecto se parecerá a la ilustración siguiente.
...
1. Abra el directorio de la **solución, c:\publish**, mediante **Explorador de archivos**.
...
3. Abra el **archivoExcelWorkbook.dll.manifest** en un editor de texto como el Bloc de notas.

4. Agregue el código siguiente detrás del elemento `</vstav3:update>`. Para el atributo de clase del `<vstav3:entryPoint>` elemento , use la sintaxis siguiente: *NamespaceName.ClassName*. En el ejemplo siguiente, los nombres de clase y espacio de nombres son los mismos, por lo que el nombre del punto de entrada resultante es `FileCopyPDA.FileCopyPDA`.
...
1. En la **carpeta %USERPROFILE%\Documents\Visual Studio 2013\Projects\ExcelWorkbook\ExcelWorkbook,** copie el archivo de certificado **ExcelWorkbook_TemporaryKey.pfx** y péguelo en la carpeta *PublishFolder* **\Application Files\ExcelWorkbook** \_ _MostRecentPublishedVersion._

2. Abra el símbolo del sistema Visual Studio y, a continuación, cambie los directorios a la carpeta **c:\publish\Application Files\ExcelWorkbook** \_ _MostRecentPublishedVersion_ (por ejemplo, **c:\publish\Application Files\ExcelWorkbook_1_0_0_4**).
...
5. Copie el *archivo ExcelWorkbook.vsto* en el directorio **c:\publish\Application Files\ExcelWorkbook** \_ _MostRecentVersionNumber._

## <a name="put-the-document-of-a-solution-onto-a-server-thats-running-sharepoint-document-level-customizations-only"></a><a name="SharePoint"></a> Coloque el documento de una solución en un servidor que ejecute SharePoint (solo personalizaciones de nivel de documento)
...
2. Siga los pasos de alguno de estos enfoques:
    ...
    - Asegúrese de que cada usuario realice los pasos siguientes.
        ...
        5. Active la **casilla Subcarpetas de esta ubicación también son** de confianza y, a continuación, elija el **botón** Aceptar.
...
## <a name="create-a-custom-installer"></a><a name="Custom"></a> Creación de un instalador personalizado
 Puede crear un instalador personalizado para la solución de Office, en lugar de usar el programa de instalación que se crea automáticamente al publicar la solución. Por ejemplo, podría usar un script de inicio de sesión para iniciar la instalación o podría usar un archivo por lotes para instalar la solución sin la interacción del usuario. Estos escenarios funcionan mejor si los requisitos previos ya están instalados en los equipos de los usuarios finales.
 ...
 *%commonprogramfiles%\microsoft shared\VSTO\10.0\VSTOInstaller.exe*
 ...
 Si la herramienta no está en esa  ubicación, puede usarHKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\VSTO Runtime Setup\v4\InstallerPathoHKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Microsoft\VSTO Runtime Setup\v4\InstallerPathclave del Registro para encontrar la ruta de acceso **a** esa herramienta.

 Puede usar los parámetros siguientes con *VSTOinstaller.exe*.

| Parámetro | Definición |
|------------------| - |
| /Install o /I | Instala la solución. Esta opción debe ir seguida de la ruta de acceso de un manifiesto de implementación. Puede especificar una ruta de acceso en el equipo local —un recurso compartido de archivos de convención de nomenclatura universal (UNC)—. Puede especificar una ruta de acceso local (*C:\FolderName\PublishFolder*), una ruta de acceso relativa (*Publicar \\*) o una ubicación completa (*\\ \ServerName\FolderName* o http://<em>ServerName/FolderName</em>). |
...
| /Silent o /S | Instala o desinstala sin solicitar la entrada de datos del usuario ni mostrar ningún mensaje. Si se requiere un símbolo del sistema de confianza, la personalización no se instala ni actualiza. |
...
|-100|Una o varias opciones de línea de comandos no son válidas o se han establecido más de una vez. Para obtener más información, escriba "vstoinstaller /?" o vea [Crear un instalador personalizado para una solución de Office ClickOnce.](/previous-versions/bb772078(v=vs.110))|
...
|-202|No se pudo instalar la solución porque la Visual Studio Tools para Office del manifiesto de aplicación no es válida. Consulte [Manifiestos de aplicación para soluciones de Office.](../vsto/application-manifests-for-office-solutions.md)|
...
|-300|No se pudo instalar la solución porque se produjo una excepción de seguridad. Consulte [Soluciones seguras de Office.](../vsto/securing-office-solutions.md)|
...
## <a name="publish-an-update"></a><a name="Update"></a> Publicación de una actualización
 ...
 La próxima vez que la solución busque una actualización, buscará y cargará automáticamente la nueva versión.
...
2. Ejecute el programa de instalación e incluya el parámetro `/url`, que toma la nueva ruta de instalación como una cadena.
   ...
   Para las personalizaciones de nivel de documento, los usuarios deben abrir el documento y, a continuación, actualizar su _AssemblyLocation propiedad. Los pasos siguientes describen cómo los usuarios pueden realizar esta tarea.
...
1. En la **pestaña Archivo,** elija **Información**, que se muestra en la ilustración siguiente.

     ![Pestaña Información de Excel](../vsto/media/vsto-infotab.png "Pestaña Información de Excel")

2. En la **lista Propiedades,** elija **Propiedades avanzadas**, que se muestra en la ilustración siguiente.

     ![Propiedades avanzadas de Excel.](../vsto/media/vsto-advanceddocumentproperties.png "Propiedades avanzadas de Excel.")

3. En la **pestaña Personalizado** de la **lista Propiedades,** elija _AssemblyLocation, como se muestra en la ilustración siguiente.
...
4. Antes del identificador, escriba la ruta de acceso completa del documento, seguida de una barra, con el formato Identificador de ruta |  de acceso (por ejemplo, *File://ServerName/FolderName/FileName|74744e4b-e4d6-41eb-84f7-ad20346fe2d9*.

     Para obtener más información sobre cómo dar formato a este identificador, vea [Información general sobre las propiedades de documento personalizado.](../vsto/custom-document-properties-overview.md)
...
2. En la carpeta publish de nivel superior, elimine el manifiesto de implementación (el *archivo .vsto).*
...
4. Copie el manifiesto de implementación de esa subcarpeta en la carpeta de publicación de nivel superior.
     ...
     Por ejemplo, para revertir una solución denominada **OutlookAddIn1** de la versión 1.0.0.1 a la versión 1.0.0.0, copie el archivo **OutlookAddIn1.vsto** de la **carpeta OutlookAddIn1_1_0_0_0.** Pegue el archivo en la carpeta de publicación de nivel superior y sobrescriba el manifiesto de implementación específico de la versión **OutlookAddIn1_1_0_0_1** que ya estaba allí.
...
- [Creación de un instalador personalizado para una solución de oficina ClickOnce](/previous-versions/bb772078(v=vs.110))