...
description: Obtenga información sobre que los paneles de tareas personalizados proporcionan una manera de crear su propio panel de tareas y proporcionar a los usuarios una interfaz conocida para acceder a las características de la solución.
...
> [!NOTE]
> Un panel de tareas personalizado difiere del panel de acciones. El panel de acciones es parte de las personalizaciones de nivel de documento de Microsoft Office Word y Microsoft Office Excel. Para obtener más información, vea Información [general del panel Acciones.](../vsto/actions-pane-overview.md)
...
## <a name="create-a-custom-task-pane"></a>Creación de un panel de tareas personalizado
...
### <a name="instantiate-the-custom-task-pane"></a>Creación de una instancia del panel de tareas personalizado
...
## <a name="access-the-application-from-the-task-pane"></a>Acceso a la aplicación desde el panel de tareas
 ...
 Para obtener más información sobre `Application` el campo del objeto , vea Program `ThisAddIn` [VSTO Add-ins](../vsto/programming-vsto-add-ins.md). Para obtener un tutorial que muestra cómo automatizar una aplicación desde un panel de tareas personalizado, vea Tutorial: Automático de una aplicación [desde un panel de tareas personalizado.](../vsto/walkthrough-automating-an-application-from-a-custom-task-pane.md) Para obtener más información sobre la `Globals` clase , vea Acceso global a objetos en proyectos de [Office.](../vsto/global-access-to-objects-in-office-projects.md)
...
### <a name="make-the-custom-task-pane-visible"></a>Hacer visible el panel de tareas personalizado
 De forma predeterminada, el panel de tareas no está visible. Para que el panel de tareas sea visible, debe establecer la <xref:Microsoft.Office.Tools.CustomTaskPane.Visible%2A> propiedad en **true.**

 Los usuarios pueden cerrar un panel de tareas en cualquier momento haciendo clic en **el** botón Cerrar (X) de la esquina del panel de tareas. Sin embargo, no hay ninguna manera predeterminada para que los usuarios vuelvan a abrir el panel de tareas personalizado. Si un usuario cierra un panel de tareas personalizado, no podrá volver a ver el panel de tareas personalizado, a menos que le proporcione la manera de mostrarlo.

 Si crea un panel de tareas personalizado en el complemento de VSTO, debe crear también un elemento de interfaz de usuario, como un botón, en el que los usuarios puedan hacer clic para mostrar u ocultar el panel de tareas personalizado. Si crea un panel de tareas personalizado en una aplicación de Microsoft Office que admite la personalización de la cinta de opciones, puede agregar un grupo de controles a la cinta con un botón que muestre u oculte el panel de tareas personalizado. Para obtener un tutorial que muestra cómo hacerlo, vea [Tutorial: Sincronizar](../vsto/walkthrough-synchronizing-a-custom-task-pane-with-a-ribbon-button.md)un panel de tareas personalizado con un botón de cinta de opciones .
...
### <a name="program-custom-task-pane-events"></a>Programación de eventos del panel de tareas personalizados
...
## <a name="clean-up-resources-used-by-the-task-pane"></a>Limpieza de los recursos usados por el panel de tareas
 Después de crear un panel de tareas personalizado, el objeto <xref:Microsoft.Office.Tools.CustomTaskPane> permanece en memoria mientras se ejecuta el complemento de VSTO. El objeto permanece en memoria incluso después de que el usuario haga clic en el botón **Cerrar** (X) de la esquina del panel de tareas.
...
## <a name="outlook"></a><a name="Outlook"></a> Outlook
 ...
 Para asociar el panel de tareas a un explorador o inspector específico, use el método para crear el panel de tareas y pase el objeto <xref:Microsoft.Office.Tools.CustomTaskPaneCollection.Add%2A> o al parámetro de <xref:Microsoft.Office.Interop.Outlook.Explorer> <xref:Microsoft.Office.Interop.Outlook.Inspector> *ventana.* Para obtener más información sobre cómo crear paneles de tareas personalizados, vea [Información general sobre paneles de tareas personalizados.](../vsto/custom-task-panes.md)
...
## <a name="word-infopath-and-powerpoint"></a><a name="WordAndInfoPath"></a> Word, InfoPath y PowerPoint
 ...
 Para asociar el panel de tareas a una ventana de documento específica, use el método para crear el panel de tareas y pase <xref:Microsoft.Office.Tools.CustomTaskPaneCollection.Add%2A> <xref:Microsoft.Office.Interop.Word.Window> (para Word), <xref:Microsoft.Office.Interop.InfoPath.WindowObject> (para InfoPath) o [DocumentWindow](/previous-versions/office/developer/office-2010/ff762047(v=office.14)) (para PowerPoint)  al parámetro de ventana.
...
- [Microsoft.Office.Interop.PowerPoint.EApplication_Event.PresentationAbrir](/previous-versions/office/developer/office-2010/ff760423(v=office.14))
...
- [Tutorial: Sincronizar un panel de tareas personalizado con un botón de cinta de opciones](../vsto/walkthrough-synchronizing-a-custom-task-pane-with-a-ribbon-button.md)
