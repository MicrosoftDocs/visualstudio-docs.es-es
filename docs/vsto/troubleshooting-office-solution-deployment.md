...
## <a name="change-the-assembly-name-causes-conflicts"></a>Cambiar el nombre del ensamblado provoca conflictos
 Si cambia el valor **de Nombre** de ensamblado en la página **Aplicación** del Diseñador de **proyectos** después de implementar una solución, las herramientas de publicación modificarán el paquete de instalación para que tenga un archivo *Setup.exe* y dos manifiestos de implementación. Si implementa dos archivos de manifiesto, podrían producirse las siguientes condiciones:
...
## <a name="check-for-updates-takes-a-long-time"></a>Buscar actualizaciones tarda mucho tiempo
 Visual Studio 2010 Tools para el tiempo de ejecución de Office proporciona una entrada del Registro que los administradores pueden usar para establecer el valor de tiempo de espera para descargar los manifiestos y la solución.
...
## <a name="prerequisites-for-microsoft-office-arent-installed"></a>Los requisitos previos para Microsoft Office no están instalados
 Puede agregar .NET Framework, [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)]y los ensamblados de Office de interoperabilidad primarios al paquete de instalación como requisitos previos que se implementan con la solución de Office. Para obtener información acerca de cómo instalar los ensamblados de interoperabilidad primarios, vea [Configurar un equipo para desarrollar soluciones](../vsto/configuring-a-computer-to-develop-office-solutions.md) de Office y [Cómo: instalar ensamblados de interoperabilidad primarios](../vsto/how-to-install-office-primary-interop-assemblies.md)de Office .
...
3. Establezca el número de revisión, el tercer campo,\*de la **versión**de ensamblaje , en un comodín ( ). Por ejemplo, "1.0.*".  A continuación, elija el botón **Aceptar.**
 ...
 [!NOTE]
> A partir de Visual Studio 2017, si intenta usar comodines en la versión de ensamblado se producirá un error de compilación.  Esto se debe a que los comodines de la versión del ensamblado romperán la característica determinista de MSBuild. Se le indicará que quite los caracteres comodín de la versión del ensamblado o deshabilite el determinismo.  Para obtener más información sobre la característica determinista, consulte: Propiedades comunes del [proyecto de MSBuild](../msbuild/common-msbuild-project-properties.md) y Personalizar la [compilación](../msbuild/customize-your-build.md)
...
## <a name="prompt-to-manually-uninstall-appears-when-you-publish-and-install-a-solution-on-the-development-computer"></a>El símbolo del sistema de instalación aparece al publicar e instalar una solución en el equipo de desarrollo
...
## <a name="uncaught-exception-or-method-not-found-error-when-you-install-a-solution"></a>Excepción no detectada o método no encontrado error al instalar una solución
 Al instalar soluciones de Office abriendo el manifiesto de implementación (un archivo *.vsto),* la aplicación de Office, el documento o el libro, pueden aparecer mensajes de error para las siguientes condiciones:
...
## <a name="manifest-registry-keys-for-add-ins-change-after-an-installshield-limited-edition-project-is-built"></a>Las claves de registro de manifiesto para complementos cambian después de crear un proyecto InstallShield Limited Edition
...
## <a name="the-clickonce-installer-for-your-office-solution-doesnt-install-the-primary-interop-assemblies"></a>El instalador ClickOnce para la solución de Office no instala los ensamblados de interoperabilidad primarios
 ...
 Si el programa de instalación no instala correctamente los PIA, instálelos manualmente ejecutando el archivo de instalación denominado *o2007pia.msi* desde el directorio de instalación.

## <a name="reinstall-office-solutions-causes-an-argument-out-of-range-exception"></a>Reinstalar soluciones de Office provoca un argumento fuera de intervalo de excepción
 ...
 Esta situación se produce si las mayúsculas y minúsculas de la dirección URL de la ubicación de instalación son diferentes. Por ejemplo, este error aparecería si `http://fabrikam.com/ExcelSolution.vsto` instaló una solución de Office desde la primera vez y, a continuación, usó `http://fabrikam.com/excelsolution.vsto` la segunda vez.
