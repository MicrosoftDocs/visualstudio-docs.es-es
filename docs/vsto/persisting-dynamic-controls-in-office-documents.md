...
## <a name="persist-host-controls-in-the-document"></a>Controles host en el documento se conservan  
...
## <a name="persist-windows-forms-controls-in-documents"></a>Se conservan los controles de formularios Windows Forms en documentos  
...
1.  Almacenar información sobre el tamaño, la ubicación y el estado de los controles cuando se guarda o se cierra el documento. En una personalización de nivel de documento, puede guardar los datos en la caché de datos en el documento. En un complemento de VSTO, puede guardar los datos en un elemento XML personalizado en el documento.  
...
#### <a name="remove-activex-wrappers-when-the-document-is-opened"></a>Quitar los contenedores de ActiveX cuando se abre el documento  
 Para quitar todos los contenedores de ActiveX, llame a la `GetVstoObject` método para generar un elemento host para el <xref:Microsoft.Office.Interop.Word.Document> o <xref:Microsoft.Office.Interop.Excel.Workbook> que representa el documento recién abierto. Por ejemplo, para quitar todos los contenedores de ActiveX de un documento de Word, puede llamar a la `GetVstoObject` método para generar un elemento host para el <xref:Microsoft.Office.Interop.Word.Document> objeto que se pasa al controlador de eventos para el <xref:Microsoft.Office.Interop.Word.ApplicationEvents4_Event.DocumentOpen> eventos.  
