---
title: Conservación de controles dinámicos en documentos de Office
description: Obtenga información sobre cómo agregar código a la solución para volver a crear controles dinámicos persistentes cuando un usuario vuelve a abrir un documento cerrado.
...
ms.locfileid: "107826153"
---
# <a name="persist-dynamic-controls-in-office-documents"></a>Conservación de controles dinámicos en documentos de Office
...
## <a name="persist-host-controls-in-the-document"></a>Conservación de los controles host en el documento
...
Para volver a crear un control host para Word o un control host o para Excel, use un <xref:Microsoft.Office.Tools.Excel.NamedRange> método de un objeto o <xref:Microsoft.Office.Tools.Excel.ListObject> `Add` \<*control class*> <xref:Microsoft.Office.Tools.Excel.ControlCollection?displayProperty=fullName> <xref:Microsoft.Office.Tools.Word.ControlCollection?displayProperty=fullName> . Utilice un método que tenga un parámetro para el objeto de Office nativo.
...
Para volver a crear un control host, primero debe eliminar el nativo y, a continuación, volver a crear mediante <xref:Microsoft.Office.Tools.Excel.Chart?displayProperty=fullName> <xref:Microsoft.Office.Interop.Excel.Chart?displayProperty=fullName> el método <xref:Microsoft.Office.Tools.Excel.Chart?displayProperty=fullName> <xref:Microsoft.Office.Tools.Excel.ControlCollection.AddChart%2A> . No hay ningún `Add` \<*control class*> método que le permita crear un nuevo basado <xref:Microsoft.Office.Tools.Excel.Chart?displayProperty=fullName> en un <xref:Microsoft.Office.Interop.Excel.Chart?displayProperty=fullName> existente.

Si no elimina primero el nativo , creará un segundo gráfico duplicado <xref:Microsoft.Office.Interop.Excel.Chart> al volver a crear . <xref:Microsoft.Office.Tools.Excel.Chart?displayProperty=fullName>

## <a name="persist-windows-forms-controls-in-documents"></a>Conservación Windows Forms controles en documentos
...
1. Almacenar información sobre el tamaño, la ubicación y el estado de los controles cuando se guarda o se cierra el documento. En una personalización de nivel de documento, puede guardar los datos en la caché de datos del documento. En un complemento de VSTO, puede guardar los datos en un elemento XML personalizado del documento.

2. Volver a crear los controles en un evento que se genera cuando se abre el documento. En los proyectos de nivel de documento, puede hacerlo en los `Sheet` *controladores de eventos n* o `_Startup` `ThisDocument_Startup` . En proyectos de complemento de VSTO, puede hacerlo en los controladores de eventos de los eventos <xref:Microsoft.Office.Interop.Excel.AppEvents_Event.WorkbookOpen> o <xref:Microsoft.Office.Interop.Word.ApplicationEvents4_Event.DocumentOpen> .

### <a name="remove-activex-wrappers-in-an-add-in"></a><a name="removingActiveX"></a> Eliminación de contenedores ActiveX en un complemento

Al agregar controles de Windows Forms dinámicos a documentos mediante un complemento de VSTO, puede evitar que los contenedores ActiveX de los controles aparezcan en el documento la próxima vez que se abra de las maneras siguientes.

#### <a name="remove-activex-wrappers-when-the-document-is-opened"></a>Quitar contenedores ActiveX cuando se abre el documento
...
Aunque el método se usa principalmente para generar un nuevo elemento host en tiempo de ejecución, este método también borra todos los contenedores ActiveX del documento la primera vez que se llama `GetVstoObject` para un documento específico. Para obtener más información sobre cómo usar el método , vea Extender documentos de Word y libros de Excel en complementos `GetVstoObject` [vsTO en tiempo de ejecución.](../vsto/extending-word-documents-and-excel-workbooks-in-vsto-add-ins-at-run-time.md)

Si el complemento de VSTO crea controles dinámicos cuando se abre el documento, el complemento de VSTO ya llamará al método como parte del proceso para `GetVstoObject` crear los controles. No es necesario agregar una llamada independiente al método `GetVstoObject` para quitar los contenedores de ActiveX en este escenario.
