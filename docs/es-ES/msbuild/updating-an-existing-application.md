# <a name="updating-an-existing-application-for-msbuild-15"></a>Actualización de una aplicación existente a MSBuild 15

En las versiones de MSBuild anteriores a 15.0, MSBuild se cargaba desde la caché global de ensamblados (GAC) y las extensiones de MSBuild se instalaban en el Registro. Esto garantizaba que todas las aplicaciones usaran la misma versión de MSBuild y tuvieran acceso a los mismos conjuntos de herramientas, pero evitaba las instalaciones en paralelo de distintas versiones de Visual Studio.
...
## <a name="using-msbuild-from-visual-studio"></a>Uso de MSBuild desde Visual Studio

Para asegurarse de que las compilaciones mediante programación de la aplicación coincidan con las realizadas en Visual Studio o MSBuild.exe, cargue los ensamblados de MSBuild desde Visual Studio y use los SDK disponibles en Visual Studio. El paquete NuGet Microsoft.Build.Locator simplifica esto.

## <a name="using-microsoftbuildlocator"></a>Uso de Microsoft.Build.Locator

Si redistribuye `Microsoft.Build.Locator.dll` con la aplicación, no necesita distribuir otros ensamblados de MSBuild.
...
Con el fin de garantizar que MSBuild se cargue desde una ubicación central, no debe distribuir sus ensamblados con la aplicación.
...
#### <a name="using-nuget-packages-preferred"></a>Uso de paquetes NuGet (preferido)

En estas instrucciones se da por hecho que usa [referencias NuGet de estilo `PackageReference`](https://docs.microsoft.com/en-us/nuget/consume-packages/package-references-in-project-files).
...
#### <a name="using-extension-assemblies"></a>Uso de ensamblados de extensión

Si no puede usar paquetes NuGet, puede hacer referencia a ensamblados de MSBuild distribuidos con Visual Studio. Si hace referencia a MSBuild directamente, asegúrese de que no se copie en el directorio de salida al establecer `Copy Local` en `False`. En el archivo del proyecto, esto tendría el aspecto siguiente:
...
Al hacer referencia al paquete de Microsoft.Build.Locator se garantiza automáticamente que la aplicación use las redirecciones de enlaces necesarias de todas las versiones de ensamblados de MSBuild a la versión `15.1.0.0`.

### <a name="ensure-output-clean"></a>Garantizar una salida limpia

Compile el proyecto y examine el directorio de salida para asegurarse de que no contiene ningún ensamblado `Microsoft.Build.*.dll` (más que `Microsoft.Build.Locator.dll`, que se agrega en el paso siguiente).
...
La manera más sencilla de hacerlo es agregar una llamada a
