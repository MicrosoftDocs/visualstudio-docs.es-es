...
Una entrada de solicitud HTTP que podría no ser <xref:System.Windows.Markup.XamlReader?displayProperty=nameWithType> de confianza llega a un método Load.
...
Esta regla intenta buscar la entrada de las solicitudes HTTP que llegan <xref:System.Windows.Markup.XamlReader?displayProperty=nameWithType> a un método Load.
