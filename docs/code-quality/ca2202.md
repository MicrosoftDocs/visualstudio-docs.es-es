...
Una implementación de método contiene rutas de acceso de código que podrían provocar varias llamadas a <xref:System.IDisposable.Dispose%2A?displayProperty=fullName> o a un equivalente de Dispose, como un método Close () en algunos tipos, en el mismo objeto.
...
Para corregir una infracción de esta regla, cambie la implementación para que independientemente de la ruta de acceso del código, <xref:System.IDisposable.Dispose%2A> solo se llame una vez para el objeto.
