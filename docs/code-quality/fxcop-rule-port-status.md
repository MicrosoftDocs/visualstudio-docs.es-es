---
title: Estado del puerto de regla FxCop
ms.date: 05/21/2019
ms.topic: reference
helpviewer_keywords:
- fxcop rules
...
ms.locfileid: "79301612"
---
# <a name="fxcop-rule-port-status"></a>Estado del puerto de regla fxcop

Si anteriormente usó el análisis de código estático en Visual Studio, es posible que se pregunte cuáles de esas reglas están disponibles en la implementación actual como [analizadores de FxCop.](install-fxcop-analyzers.md) Esta página enumera las reglas que se portan, así como las que no se han portado y si hay planes para portarlas.

## <a name="ported-rules"></a>Reglas migradas

La página de [documentación generada automáticamente](https://github.com/dotnet/roslyn-analyzers/blob/master/src/Microsoft.CodeAnalysis.FxCopAnalyzers/Microsoft.CodeAnalysis.FxCopAnalyzers.md) en el repositorio roslyn-analyzers tiene la lista más actualizada de reglas que se han portado a los analizadores FxCop. Esa página también tiene información adicional, como si la regla está habilitada de forma predeterminada y si tiene una *corrección*de código asociada. ([Las correcciones](../ide/quick-actions.md) de código son correcciones de un solo clic disponibles en el menú de iconos de bombillas en Visual Studio.)

A partir de la fecha en esta página, la lista de reglas de FxCop que se han portado a [los analizadores FxCop](install-fxcop-analyzers.md) incluye:
...
[CA1014](ca1014-mark-assemblies-with-clscompliantattribute.md) | Marcar ensamblajes con CLSCompliant
[CA1016](ca1016-mark-assemblies-with-assemblyversionattribute.md) | Marcar ensamblajes con versión de ensamblaje
[CA1017](ca1017-mark-assemblies-with-comvisibleattribute.md) | Marcar ensamblajes con ComVisible
...
[CA1028](ca1028-enum-storage-should-be-int32.md) | Enum Storage debe ser Int32
...
[CA1043](ca1043-use-integral-or-string-argument-for-indexers.md) | Usar argumento integral o de cadena para los indexadores
...
[CA1052](ca1052-static-holder-types-should-be-sealed.md) | Los tipos de soporteestático deben ser estáticos o no heredables
[CA1053](ca1053-static-holder-types-should-not-have-constructors.md) | Los tipos de soporte estático no deben tener constructores (CA1053 forma parte de [CA1052](ca1052-static-holder-types-should-be-sealed.md) para analizadores FxCop)
[CA1054](ca1054-uri-parameters-should-not-be-strings.md) | Los parámetros uri no deben ser cadenas
[CA1055](ca1055-uri-return-values-should-not-be-strings.md) | Los valores devueltos de Uri no deben ser cadenas
[CA1056](ca1056-uri-properties-should-not-be-strings.md) | Las propiedades de Uri no deben ser cadenas
[CA1058](ca1058-types-should-not-extend-certain-base-types.md) | Los tipos no deben ampliar ciertos tipos base
[CA1060](ca1060-move-p-invokes-to-nativemethods-class.md) | Mover pinvokes a la clase de métodos nativos
...
CA1066 | El {0} tipo debe implementar\<el> T iEquatable porque invalida igual
...
[CA1507](ca1507.md) | Utilice nameof para expresar los nombres de los símbolos
CA1508 | Evitar el código condicional muerto
...
[CA1720](ca1720-identifiers-should-not-contain-type-names.md) | El identificador contiene el nombre del tipo
...
[CA1724](ca1724-type-names-should-not-match-namespaces.md) | Los nombres de tipo no deben coincidir con espacios de nombres
...
[CA1802](ca1802.md) | Utilice literales cuando sea apropiado
...
[CA1821](ca1821.md) | Eliminar los finalizadores vacíos
...
CA1826 | No utilice métodos Enumerable en colecciones indexables. En su lugar, utilice la colección directamente
[CA2000](ca2000.md) | Desechar objetos antes de perder el ámbito
[CA2002](ca2002.md) | No bloquear objetos con identidad débil
[CA2007](ca2007.md) | Considere llamar a ConfigureAwait en la tarea esperada
CA2008 | No cree tareas sin pasar un TaskScheduler
...
[CA2153](ca2153.md) | No capture excepciones de estado dañadas
[CA2200](ca2200.md) | Vuelva a escribir para conservar los detalles de la pila.
...
[CA2219](ca2219.md) | No plantee excepciones en las cláusulas finally
[CA2224](ca2224.md) | Reemplazar es igual a la sobrecarga del operador es igual a
...
[CA2231](ca2231.md) | El operador de sobrecarga es igual a la invalidación del tipo de valor Equals
...
CA2244 | No duplique las inicializaciones de elementos indexados
...
CA3061 | No agregar esquema por URL
[CA3075](ca3075.md) | Procesamiento de DTD no seguro en XML
[CA3076](ca3076.md) | Procesamiento de scripts XSLT no seguro.
[CA3077](ca3077.md) | Procesamiento inseguro en API Design, XmlDocument y XmlTextReader
[CA3147](ca3147.md) | Marcar manipuladores de verbos con token antifalsificación validado
[CA5350](ca5350.md) | No usar algoritmos criptográficos no seguros
[CA5351](ca5351.md) | No utilice algoritmos criptográficos rotos
...
CA5361 | No deshabilite el uso de SChannel de Crypto fuerte
CA5362 | No se refiera a sí mismo en la clase serializable
CA5363 | No deshabilitar la validación de solicitudes
CA5364 | No utilice protocolos de seguridad obsoletos
CA5365 | No deshabilite la comprobación de encabezado HTTP
CA5366 | Usar XmlReader para DataSet Read Xml
CA5367 | No serializar tipos con campos de puntero
CA5368 | Establecer ViewStateUserKey para las clases derivadas de la página
CA5369 | Usar XmlReader para deserializar
CA5370 | Utilice XmlReader para validar el lector
CA5371 | Usar XmlReader para lectura de esquemas
...
CA5375 | No use la firma de acceso compartido de la cuenta
CA5376 | Usar SharedAccessProtocol HttpsOnly
CA5377 | Usar directiva de acceso a nivel de contenedor
CA5378 | No deshabilitar ServicePointManagerSecurityProtocols
CA5379 | No utilice el algoritmo de función de derivación de clave débil
CA9999 | Desajuste de la versión del analizador

## <a name="unported-rules"></a>Reglas no portadas

El conjunto de reglas que no se ha portado a [los analizadores FxCop](install-fxcop-analyzers.md) consta de reglas que aún no se han portado, pero que todavía [pueden ser porteadas,](#rules-that-may-be-ported)y las que están en desuso y [no se porten.](#deprecated-rules)

### <a name="rules-that-may-be-ported"></a>Reglas que pueden ser portadas

Las siguientes reglas de análisis heredadas de FxCop aún no se han implementado como analizadores, pero aún así pueden serlo. Esto podría deberse a una razón técnica de bloqueo o simplemente a que la regla es de menor prioridad. Para obtener más información sobre el estado de portabilidad de cada regla, haga clic en el vínculo de la columna Problema de **seguimiento.**
...
Las siguientes reglas de análisis heredadas de FxCop están en desuso y no se implementarán como analizadores. Para obtener más información, puede buscar por identificador de regla (por ejemplo, **CA1009**) en la página de problemas de [GitHub roslyn-analyzers](https://github.com/dotnet/roslyn-analyzers/issues?utf8=%E2%9C%93&q=is:issue+label:FxCop-Port).
