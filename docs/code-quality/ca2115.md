...
Un método declarado en un tipo con un finalizador hace referencia a un campo <xref:System.IntPtr?displayProperty=fullName> o <xref:System.UIntPtr?displayProperty=fullName>, pero no llama a <xref:System.GC.KeepAlive%2A?displayProperty=fullName>.
...
- El finalizador no libera el contenido de los campos <xref:System.IntPtr> o <xref:System.UIntPtr> a los que hace referencia el método.

- El método no pasa el campo <xref:System.IntPtr> o <xref:System.UIntPtr> a código no administrado.
