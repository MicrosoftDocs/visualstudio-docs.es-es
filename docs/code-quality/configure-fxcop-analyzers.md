...
ms.locfileid: "90035435"
---
# <a name="configure-net-code-quality-analysis-with-editorconfig"></a>Configurar el análisis de calidad del código de .NET con EditorConfig

Cada analizador de calidad de código (los que empiezan por los identificadores de regla `CA` ) se puede refinar para que se aplique a las partes del código base a través de opciones configurables. Cada opción se especifica agregando un par clave-valor a un archivo [EditorConfig](https://editorconfig.org) . Un archivo de configuración puede ser específico de un archivo, proyecto, solución o todo el repositorio.
...
Para habilitar la configuración, agregue un archivo *. editorconfig* con las opciones del directorio correspondiente. Este archivo también puede contener entradas de configuración de gravedad de diagnóstico basadas en EditorConfig. Obtenga más información [aquí](use-roslyn-analyzers.md#configure-severity-levels).
