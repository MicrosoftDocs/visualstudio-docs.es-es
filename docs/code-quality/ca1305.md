...
Para corregir una infracción de esta regla, use la sobrecarga que toma un <xref:System.IFormatProvider> argumento. O bien, use una [cadena interpolada de C#](/dotnet/csharp/tutorials/string-interpolation) y páselo al <xref:System.FormattableString.Invariant%2A?displayProperty=nameWithType> método.
