...
Para asegurarse de que no expone una superficie expuesta a ataques, esta regla se desencadena cuando el valor de XslCompiledTransform. <xref:System.Xml.Xsl.XslCompiledTransform.Load%2A> recibe instancias de combinación no seguras de <xref:System.Xml.Xsl.XsltSettings> y <xref:System.Xml.XmlResolver>, lo que permite el procesamiento de scripts malintencionados.
...
### <a name="violationmdashdocument-function-and-script-execution-not-disabled"></a>Infracción @ no__t-0document función y ejecución de script no deshabilitada
...
### <a name="solutionmdashdisable-document-function-and-script-execution"></a>Solución @ no__t-0disable de la función de documento y ejecución de script
