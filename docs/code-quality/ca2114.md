...
Un método no debe tener una seguridad declarativa de nivel de método y de nivel de tipo para la misma acción. Las dos comprobaciones no se combinan; solo se aplica la demanda de nivel de método. Por ejemplo, si un tipo solicita el permiso `X` y uno de sus métodos exige el permiso `Y`, el código no tiene que tener permiso @no__t 2 para ejecutar el método.

## <a name="how-to-fix-violations"></a>Cómo corregir infracciones
Revise el código para asegurarse de que se requieren ambas acciones. Si se requieren ambas acciones, asegúrese de que la acción de nivel de método incluye la seguridad especificada en el nivel de tipo. Por ejemplo, si el tipo exige el permiso `X` y su método también debe solicitar el permiso `Y`, el método debe solicitar explícitamente `X` y `Y`.
