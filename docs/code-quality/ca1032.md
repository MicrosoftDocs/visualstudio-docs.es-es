...
Es seguro suprimir una advertencia de esta regla cuando se produce la infracción mediante el uso de un nivel de acceso diferente para los constructores públicos. Además, se puede suprimir la advertencia del constructor `NewException(SerializationInfo, StreamingContext)` si se va a compilar una biblioteca de clases portable (PCL).
