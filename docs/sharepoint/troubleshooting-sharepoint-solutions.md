...
# Solucionar problemas de soluciones de SharePoint
  Los siguientes problemas o alertas pueden producirse al depurar las soluciones de SharePoint mediante el depurador de [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)].  Para obtener más información, vea [NIB: Debugging SharePoint 2007 Workflow Solutions](http://msdn.microsoft.com/es-es/3a5392f3-66f3-48be-956e-02de23fa6247).  
...
## Tras la implementación no aparecen en la página de creación del sitio las definiciones del sitio en idiomas distintos del inglés  
 Después de crear e implementar una nueva definición del sitio con una versión de [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] que no es para el inglés \(es decir, una versión con un [!INCLUDE[TLA2#tla_id](../sharepoint/includes/tla2sharptla-id-md.md)] de configuración regional distinto del 1033\), la pestaña **Personalizaciones de SharePoint** no aparece en el cuadro **Selección de plantilla** y la nueva plantilla del sitio no aparece en la página **Nuevo Sitio de SharePoint**.  
  
### Mensaje de error  
 Ninguno.  
  
### Resolución  
 Este problema se produce debido a un valor incorrecto de la propiedad **Path** en el archivo webtemp de configuración de definición del sitio, como webtemp\_SiteDefinitionProject1.xml.  En la propiedad **Path** del archivo webtemp, situado bajo **Ubicación de implementación**, cambie 1033 por el [!INCLUDE[TLA2#tla_id](../sharepoint/includes/tla2sharptla-id-md.md)] de configuración regional adecuado.  Por ejemplo, para usar la configuración regional del japonés, cambie el valor a 1041.  Para obtener más información, vea la información sobre [identificadores de configuración regional asignados por Microsoft](http://go.microsoft.com/fwlink/?LinkID=165561) en el sitio web de MSDN.  
...
## Error: La ubicación del sitio de SharePoint no es válida  
 Este problema puede producirse si no se instala [!INCLUDE[moss_14_short](../sharepoint/includes/moss-14-short-md.md)].  También puede ocurrir si no se tiene acceso de administrador al sitio web de SharePoint especificado en el **Asistente para personalización de SharePoint**.  
...
-   Asegúrese de que tiene acceso de administrador al sitio web de SharePoint.  Para obtener más información, vea el artículo de [!INCLUDE[TLA2#tla_office](../sharepoint/includes/tla2sharptla-office-md.md)] Online [Conceder acceso al sitio de portal](http://go.microsoft.com/fwlink/?LinkId=98310).  
...
1.  En el **Explorador de soluciones**, abra el archivo .feature del receptor de eventos en el **Diseñador de características**, haciendo doble clic en el archivo o abriendo el menú contextual y después eligiendo **Abrir**.  
...
-   El IMetadataObject denominado '\<*model name*\>' tiene un valor en el campo 'nombre' que está duplicado...  
...
-   Administración central de SharePoint 2010.  Para obtener más información, vea [Administración de modelos de Conectividad a datos empresariales \(SharePoint Server 2010\)](http://go.microsoft.com/fwlink/?LinkID=181472) en el sitio web de Microsoft TechNet.  
  
-   Windows PowerShell.  Para eliminar el modelo, escriba este comando en el símbolo del sistema: **Remove\-SPBusinessDataCatalogModel**.  Para obtener más información, vea [Cmdlets generales \(SharePoint Server 2010\)](http://go.microsoft.com/fwlink/?LinkID=182375) en el sitio web de Microsoft TechNet.  
...
4.  Asegúrese de que el valor de propiedad **Ruta de acceso** se inicia con la cadena "CONTROLTEMPLATES\\".  
...
## IIS finaliza la sesión de depuración de SharePoint  
 Este problema se produce si se establece un punto de interrupción en una solución de SharePoint para [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)], se elige la tecla F5 para ejecutarlo y, a continuación, se permanece en un punto de interrupción durante más de 90 segundos.  
