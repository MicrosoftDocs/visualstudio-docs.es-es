...
## <a name="localize-visual-studio-sharepoint-solutions"></a>Localizar soluciones de SharePoint de Visual Studio
 Al localizar una solución, debe tener en cuenta toda la información textual que la solución muestra a los usuarios. Los mensajes informativos, los mensajes de error y [!INCLUDE[TLA2#tla_ui](../sharepoint/includes/tla2sharptla-ui-md.md)] las cadenas deben traducirse y las traducciones colocadas en los archivos de recursos.
...
### <a name="localize-aspx-page-markup"></a>Localizar el marcado de página ASPX
 ...
 Establezca la propiedad **tipo de implementación** de cada archivo de recursos en **AppGlobalResource**. Esto hace que los archivos de recursos se implementen en la carpeta App_GlobalResources, donde están disponibles para todos los controles y páginas ASPX de la solución. La carpeta de App_GlobalResources se encuentra en C:\inetpub\wwwroot\wss\VirtualDirectories \\<número de puerto \> \ App_GlobalResources.
...
### <a name="localize-code"></a>Localizar código
 Además de localizar cadenas y marcado de características [!INCLUDE[vstecasp](../sharepoint/includes/vstecasp-md.md)] , también debe localizar las cadenas de mensajes y las cadenas de error que aparecen en el código de la solución. Los mensajes informativos y de error localizados se encuentran en ensamblados satélite. Los ensamblados satélite contienen cadenas que son visibles para los usuarios, como [!INCLUDE[TLA2#tla_ui](../sharepoint/includes/tla2sharptla-ui-md.md)] los mensajes de texto y salida, como las excepciones.

 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)]usa el modelo de concentrador y radio de .NET Framework estándar. El concentrador, o ensamblado del programa principal, contiene los recursos de idioma predeterminados. Los radios, o ensamblados satélite, contienen los recursos específicos del lenguaje. Para obtener más información, vea [Packaging and Deploying Resources](/previous-versions/dotnet/netframework-4.0/sb6a8618(v=vs.100)) (Empaquetar e implementar recursos). Los ensamblados satélite se compilan a partir de archivos de recursos (*. resx*). Al agregar archivos de recursos específicos del lenguaje al proyecto y al paquete de la solución, [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] compila los archivos de recursos en ensamblados satélite denominados *{nombre del proyecto} .resources.dll*.
 ...
 Para crear los ensamblados satélite, compile el proyecto y, a continuación, agregue los archivos como ensamblados adicionales a través de la pestaña **Opciones avanzadas** del **Diseñador de paquetes**. Al agregar los ensamblados, anteponga una carpeta de referencia cultural [!INCLUDE[TLA2#tla_id](../sharepoint/includes/tla2sharptla-id-md.md)] a la ruta de acceso de ubicación, como, por ejemplo, *DE-de \\ {nombre de elemento de proyecto} .resources.dll*. Esto permite que el paquete contenga archivos con el mismo nombre.

 En el código, reemplace las cadenas codificadas de forma rígida por llamadas al <xref:System.Web.HttpContext.GetGlobalResourceObject%2A> método con la siguiente sintaxis:
