...
Actualmente, Live Unit Testing no funciona con .NET Core. Estamos trabajando para agregar esta compatibilidad en el futuro. 
...
- Si utiliza pruebas basadas en MSTest en los proyectos, asegúrese de quitar la referencia a `Microsoft.VisualStudio.QualityTools.UnitTestFramework` y agregar referencias a los paquetes NuGet de MSTest más recientes, `MSTest.TestAdapter` (se requiere la versión preliminar 1.1.4 como mínimo) y `MSTest.TestFramework` (se requiere la versión preliminar 1.0.5 como mínimo). Para más información, consulte la sección "Marcos de pruebas admitidos" del tema [Live Unit Testing con Visual Studio 2017](live-unit-testing.md#supported-test-frameworks).
...
Hay un error conocido que puede provocar que las compilaciones de Live Unit Testing no inserten los datos de encabezado PE de Win32 siguientes: 
...
Actualmente, Live Unit Testing no funciona bien con la característica Carga de solución ligera si aún no se han cargado todos los proyectos de la solución. Puede obtener información de cobertura incorrecta en tales escenarios.
...
Se trata de un problema conocido que no hemos podido corregir en la versión de Visual Studio 2017. Se debe corregir en una actualización posterior de Visual Studio 2017. 
...
Se trata de un problema conocido. Para solucionar este problema de forma alternativa, debe realizar una edición en cualquier archivo después de haber incluido o excluido las pruebas.  
