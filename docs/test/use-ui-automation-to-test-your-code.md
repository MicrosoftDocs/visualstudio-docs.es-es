...
description: Obtenga información sobre cómo crear una prueba automatizada de IU en Visual Studio realizando la prueba manualmente mientras el generador de pruebas automatizadas de IU se ejecuta en segundo plano.
...
Para obtener más información sobre las configuraciones y las plataformas compatibles con las pruebas automatizadas de IU, vea [Plataformas compatibles](../test/supported-configurations-and-platforms-for-coded-ui-tests-and-action-recordings.md).

## <a name="install-the-coded-ui-test-component"></a>Instalar el componente Prueba automatizada de IU

Para acceder a las plantillas y las herramientas de pruebas automatizadas de IU, instale el componente **Prueba automatizada de IU** de Visual Studio.
...
## <a name="create-a-coded-ui-test"></a>Crear una prueba automatizada de IU

1. Cree un proyecto de prueba automatizada de IU.

   Las pruebas automatizadas de IU deben incluirse en un proyecto de prueba automatizada de IU. Si aún no tiene un proyecto de este tipo, cree uno. Elija **Archivo** > **Nuevo** > **Proyecto**. Busque la plantilla de proyecto **Proyecto de prueba automatizada de IU** y selecciónela.
   ...
   > [!NOTE]
   > Si no ve la plantilla **Proyecto de prueba automatizada de IU**, tiene que [instalar el componente Prueba automatizada de IU](../test/use-ui-automation-to-test-your-code.md#install-the-coded-ui-test-component).

2. Agregue un archivo de prueba automatizada de IU.

     Si acaba de crear un proyecto de IU codificada, el primer archivo de CUIT se agrega automáticamente. Para agregar otro archivo de prueba, abra el menú contextual del proyecto de prueba automatizada de IU en el **Explorador de soluciones** y luego seleccione **Agregar** > **Prueba automatizada de IU**.
     ...
     ![Cuadro de diálogo Generar código para prueba automatizada de IU](media/generate-code-for-coded-ui-test.png)
...
3. Grabe una secuencia de acciones.
     ...
     **Para iniciar la grabación**, elija el icono **Grabar**. Realice las acciones que desea probar en la aplicación, incluido el inicio de la aplicación si es necesario. Por ejemplo, si va a probar una aplicación web, puede iniciar un explorador, ir al sitio web e iniciar sesión en la aplicación.
     ...
     **Para generar código** que replique las acciones, seleccione el icono **Generar código** y escriba un nombre y una descripción para el método de prueba automatizada de IU.
...
8. Ejecute la prueba.
   ...
   Use el Explorador de pruebas o abra el menú contextual del método de prueba y después elija **Ejecutar pruebas**. Para obtener más información sobre cómo ejecutar pruebas, vea [Ejecutar pruebas unitarias con el Explorador de pruebas](../test/run-unit-tests-with-test-explorer.md) y *Opciones adicionales para ejecutar pruebas automatizadas de IU* en la sección [Pasos adicionales](#whats-next) del final de este tema.

Las secciones restantes de este tema proporcionan más detalles sobre los pasos de este procedimiento.

Para obtener un ejemplo más detallado, vea [Tutorial: Crear, editar y mantener una prueba automatizada de IU](../test/walkthrough-creating-editing-and-maintaining-a-coded-ui-test.md). En el tutorial se crea una aplicación de Windows Presentation Foundation (WPF) simple para mostrar cómo crear, modificar y mantener una prueba automatizada de IU. El tutorial proporciona soluciones para la corrección de pruebas que han sido interrumpidas por diversos problemas de sincronización y de control de refactorización.
...
Para generar el código de las aserciones y agregar el control a la asignación de la interfaz de usuario, elija el icono **Generar código**. Escriba un nombre y una descripción para el método de prueba automatizada de IU, que se agregan como comentarios para el método. Elija **Agregar y generar**. A continuación, elija el icono **Cerrar** para cerrar el **Generador de pruebas de IU codificadas**. Esto genera código similar al siguiente. Por ejemplo, si el nombre que escribió es `AssertForAddTwoNumbers`, el código tendrá el aspecto mostrado en este ejemplo:

- Agrega una llamada al método de aserción AssertForAddTwoNumbers al método de prueba del archivo de prueba automatizada de IU:
...
- Agrega un método denominado `AssertForAddTwoNumbers` a la asignación de interfaz de usuario (*UIMap.uitest*). Este archivo se abre en el **Editor de pruebas automatizadas de IU**, donde puede editar las aserciones.
     ...
     ![Editar aserción con el Editor de pruebas automatizadas de IU](../test/media/cuit_editor_assert.png)
...
Si es necesario, se puede configurar la asignación de teclado predeterminada de **Ctrl**+**Mayús**+**R** que se usa para aplicar eventos de mantener el mouse en las pruebas automatizadas de IU de modo que emplee otras teclas.
...
Después de crear la prueba automatizada de IU, puede editarla mediante cualquiera de las siguientes herramientas de Visual Studio:
...
- **Editor de código:**
  ...
  - Después de crear una prueba automatizada de IU, puede modificarla para que esté controlada por datos. Para obtener más información, vea [Crear una prueba automatizada de IU controlada por datos](../test/creating-a-data-driven-coded-ui-test.md).

  - En la reproducción de una prueba automatizada de IU, puede indicarle a esta que espere a que se produzcan ciertos eventos, como que aparezca una ventana, que desaparezca la barra de progreso, etc. Para ello, agregue el método UITestControl.WaitForControlXXX() adecuado. Para obtener una lista completa de los métodos disponibles, vea [Hacer que las pruebas automatizadas de IU esperen eventos concretos durante la reproducción](../test/making-coded-ui-tests-wait-for-specific-events-during-playback.md). Para obtener un ejemplo de una prueba automatizada de IU que espera a que se habilite un control mediante el método WaitForControlEnabled, vea [Tutorial: Crear, editar y mantener una prueba automatizada de IU](../test/walkthrough-creating-editing-and-maintaining-a-coded-ui-test.md).
...
- Declaraciones en *UIMap.uitest*.
    ...
    Puede agregar varias pruebas automatizadas de IU y varios objetos y archivos de asignación de IU para facilitar las pruebas de una aplicación de gran tamaño. Para obtener más información, vea [Probar una aplicación grande con varios mapas de IU](../test/testing-a-large-application-with-multiple-ui-maps.md).
...
Al trabajar con controles de prueba de IU en pruebas automatizadas de IU, se dividen en dos partes: acciones y propiedades.

- La primera parte consta de las acciones que se pueden realizar en los controles de prueba de IU. Por ejemplo, las pruebas automatizadas de IU pueden simular clics del mouse en un control de prueba de IU o teclas presionadas en el teclado que afecten a un control de prueba de IU.

- La segunda parte consiste en habilitar al usuario para obtener y establecer las propiedades de un control de prueba de IU. Por ejemplo, las pruebas automatizadas de IU pueden obtener el número de elementos de un objeto `ListBox` o establecer uno `CheckBox` en el estado seleccionado.
...
Puede analizar pruebas automatizadas de IU mediante los registros de dichas pruebas. Los registros de pruebas automatizadas de IU filtran y registran información importante sobre las series de pruebas automatizadas de IU. El formato de los registros permite depurar los problemas rápidamente. Para obtener más información, vea [Análisis de pruebas automatizadas de IU mediante los registros de pruebas automatizadas de IU](../test/analyzing-coded-ui-tests-using-coded-ui-test-logs.md).
...
- [Ejecutar pruebas en el proceso de compilación](/azure/devops/pipelines/test/getting-started-with-continuous-testing?view=vsts&preserve-view=true)

- [Cómo: Configurar el agente de pruebas para ejecutar pruebas que interactúen con el escritorio](/previous-versions/ee291332(v=vs.140))

**Agregar compatibilidad con controles personalizados:**  El marco de pruebas automatizadas de IU no admite todas las IU posibles, así que puede que no sea compatible con la interfaz de usuario que quiere probar. Por ejemplo, no puede crear inmediatamente una prueba automatizada de IU de la interfaz de usuario para Microsoft Excel. Pero puede crear una extensión del marco de pruebas automatizadas de IU que admita un control personalizado.
...
- [Grabar y reproducir pruebas manuales](/azure/devops/test/mtm/record-play-back-manual-tests?view=vsts&preserve-view=true)
