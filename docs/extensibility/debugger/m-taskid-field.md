---
title: "m_taskId campo | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "vs-ide-sdk"
ms.tgt_pltfrm: ""
ms.topic: "article"
helpviewer_keywords: 
  - "campo m_taskId, clase de tarea [motores de depuración de .NET Framework]"
ms.assetid: 9e4b827e-7c41-4fa3-b646-830aba2ec736
caps.latest.revision: 8
ms.author: "gregvanl"
manager: "ghogen"
caps.handback.revision: 8
---
# m_taskId campo
[!INCLUDE[vs2017banner](../../code-quality/includes/vs2017banner.md)]

El campo de respaldo para la <xref:System.Threading.Tasks.Task.Id%2A?displayProperty=fullName> propiedad.  
  
 **Espacio de nombres:** <xref:System.Threading.Tasks?displayProperty=fullName>  
  
 **Ensamblado:** mscorlib \(en mscorlib.dll\)  
  
 Porque no se puede tener acceso a este miembro interno de .NET Framework, la sintaxis siguiente se proporciona el lenguaje intermedio en común \(CIL\).  
  
## Sintaxis  
  
```  
.field private int32 m_taskId  
```  
  
## Comentarios  
  
## Vea también  
 [Clase de tarea](../../extensibility/debugger/task-class-internal-members.md)