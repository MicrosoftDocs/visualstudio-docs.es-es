...
`tokParent`\
[en] Token para el atributo primario.

`pstrName`\
[en] Nombre del módulo.

`cBuffer`\
[en] Número de bytes necesarios para la salida. `buffer`

`pcBuffer`\
[fuera] Longitud de `buffer`la salida .

`buffer`\
[fuera] Matriz que contiene los símbolos.
