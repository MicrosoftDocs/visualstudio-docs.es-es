...
Visual Studio permite la carga de VSPackages cuando se activan determinados <xref:Microsoft.VisualStudio.Shell.UIContext>s conocidos. Sin embargo, estos contextos de la interfaz de usuario no son precisos, lo que deja a los autores de la extensión ninguna opción, pero para elegir un contexto de interfaz de usuario disponible que se active antes del punto en el que realmente querían que se cargase el VSPackage. Para obtener una lista de contextos de interfaz de usuario conocidos, vea <xref:Microsoft.VisualStudio.Shell.KnownUIContexts>.
...
5. Establezca un punto de interrupción en el método <xref:Microsoft.VisualStudio.Shell.Package.Initialize%2A>.
...
|ActiveProjectOutputType:\<outputType>|El término es true cuando el tipo de salida del proyecto activo coincide exactamente.  OutputType podría ser un entero o un tipo de <xref:Microsoft.VisualStudio.Shell.Interop.__VSPROJOUTPUTTYPE>.|
...
El constructor <xref:Microsoft.VisualStudio.Shell.ProvideExtensibleUIContextRuleAttribute.%23ctor%2A> documentación muestra la sintaxis de las reglas de contexto de la interfaz de usuario extensible.
