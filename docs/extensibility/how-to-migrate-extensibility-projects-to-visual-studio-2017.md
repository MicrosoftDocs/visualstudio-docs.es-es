...
Si desea depurar la extensión en una instancia experimental de Visual Studio, asegúrese de que la acción de configuración del proyecto para **Depurar** > **iniciar** tiene el **programa externo de Inicio:** valor establecido en el archivo *devenv. exe* de la instalación de Visual Studio 2017.
...
Al buscar las dependencias, verá que una dependencia podría asignarse a varios componentes. Para determinar las dependencias que se deben especificar como requisitos previos, se recomienda elegir un componente que tenga una funcionalidad similar a la de la extensión y también tener en cuenta a los usuarios y qué tipo de componentes probablemente hayan instalado o no debería tener en cuenta la instalación de. También se recomienda crear las extensiones de una forma en la que los requisitos previos necesarios cumplan solo el mínimo que permitirá la ejecución de la extensión y que las características adicionales las tengan inactivas si no se detectan determinados componentes.
