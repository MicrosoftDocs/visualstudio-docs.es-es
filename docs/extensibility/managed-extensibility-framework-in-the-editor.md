...
## <a name="overview-of-the-managed-extensibility-framework"></a>Información general sobre el Managed Extensibility Framework
 ...
 MEF está incluido en el ensamblado *System. ComponentModel. Composition. dll* de .NET Framework versión 4.
...
- Ser consumido por otro componente
  ...
  El contenedor de composición, <xref:System.ComponentModel.Composition.Hosting.CompositionContainer>, suele ser propiedad del host. El contenedor de composición mantiene un *Catálogo* de componentes exportados.

### <a name="export-and-import-component-parts"></a>Exportar e importar partes de componentes
 Puede exportar cualquier funcionalidad, siempre que se implemente como una clase pública o un miembro público de una clase (propiedad o método). No tiene que derivar la parte del componente de <xref:System.ComponentModel.Composition.Primitives.ComposablePart>. En su lugar, debe agregar un atributo <xref:System.ComponentModel.Composition.ExportAttribute> a la clase o al miembro de clase que desea exportar. Este atributo especifica el *contrato* por el que otra parte del componente puede importar su funcionalidad.

### <a name="the-export-contract"></a>El contrato de exportación
 El <xref:System.ComponentModel.Composition.ExportAttribute> define la entidad (clase, interfaz o estructura) que se está exportando. Normalmente, el atributo export toma un parámetro que especifica el tipo de la exportación.
...
```
 ...
 De forma predeterminada, el atributo <xref:System.ComponentModel.Composition.ExportAttribute> define un contrato que es el tipo de la clase de exportación.
...
```
 ...
 En el ejemplo, el atributo `[Export]` predeterminado es equivalente a `[Export(typeof(TestAdornmentLayerDefinition))]`.
...
1. Agregue referencias a *System. Composition. ComponentModel. dll*, que se encuentra en la caché de ensamblados global (GAC), y a los ensamblados del editor.
...
3. Agregue el atributo `[Import]` a la interfaz de servicio, como se indica a continuación.
...
5. Cuando haya compilado el ensamblado, colóquelo en el *.. \Common7\IDE\Components \* carpeta de la instalación de Visual Studio.
