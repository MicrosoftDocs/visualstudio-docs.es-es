...
5. Invalide el <xref:Microsoft.VisualStudio.Shell.OleMenuCommand.DynamicItemMatch%2A> método para que llame al predicado matches <xref:Microsoft.VisualStudio.Shell.OleMenuCommand.MatchedCommandId%2A> y establezca la propiedad:
...
2. Agregue el `OnBeforeQueryStatusDynamicItem` controlador de eventos. Este es el controlador al que se `QueryStatus` llama antes de un evento. Determina si el elemento de menú es un elemento "real", es decir, no es el elemento de marcador de posición y si el elemento ya está activado (lo que significa que el proyecto ya está establecido como proyecto de inicio).
