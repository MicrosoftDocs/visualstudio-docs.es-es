...
3. Abra *TestUniversalProject.CS* y agregue las siguientes directivas de `using`:
...
4. En la clase `TestUniversalProject`, agregue un campo privado que apunte a la ventana de **salida** .
...
6. Quite el código existente del método `ShowMessageBox`:
...
8. Busque el proyecto compartido. El proyecto compartido es un contenedor puro; no genera ni genera salidas. El método siguiente busca el primer proyecto compartido en la solución buscando el objeto de <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchy> que tiene la capacidad de proyecto compartido.
...
9. En el método `ShowMessageBox`, genere el título (el nombre del proyecto que aparece en el **Explorador de soluciones**) del proyecto compartido.
...
11. En el método `ShowMessageBox`, se genera el título del proyecto de plataforma activa.
...
12. Recorra en iteración los proyectos de la plataforma. El método siguiente obtiene todos los proyectos de importación (plataforma) del proyecto compartido.
    ...
    > [!IMPORTANT]
    > Si el usuario ha abierto un C++ proyecto de aplicación universal de Windows en la instancia experimental, el código anterior produce una excepción. Se trata de un problema conocido. Para evitar la excepción, reemplace el `foreach` bloque anterior por lo siguiente:
...
13. En el método `ShowMessageBox`, se genera el título de cada proyecto de plataforma. Inserte el código siguiente después de la línea que genera el título del proyecto de plataforma activa. En esta lista solo aparecen los proyectos de plataforma que se cargan.
...
15. En el método `ShowMessageBox`, cambie el proyecto de plataforma activa. Inserte este código dentro del bloque `foreach`.
...
16. Pruébelo ahora. Presione F5 para iniciar la instancia experimental. Cree un C# proyecto de aplicación de Hub universal en la instancia experimental (en el cuadro de diálogo **nuevo proyecto** , **Visual C#**   > **Windows**  > **Windows 8**  > **aplicación de Hub**de 0**universal** ). Una vez cargada la solución, vaya al menú **herramientas** y haga clic en **invocar TestUniversalProject**y, a continuación, compruebe el texto en el panel de **salida** . Debería ver algo parecido a lo siguiente:
...
2. En el método `ShowMessageBox`, agregue el código siguiente para recorrer los elementos de la jerarquía del proyecto de plataforma. Insértelo dentro del bloque `foreach`.
...
4. Pruébelo ahora. Presione **F5** para iniciar la instancia experimental. Cree un C# proyecto de aplicación de Hub universal en la instancia experimental (en el cuadro de diálogo **nuevo proyecto** , **Visual C#**   > **Windows**  > **Windows 8**  > **aplicación de Hub**de 0**universal** ) vaya al menú **herramientas** y haga clic en **invocar TestUniversalProject**y, a continuación, compruebe el texto en el panel de **salida** . Debería ver algo parecido a lo siguiente:
...
4. Haga que la clase `HierarchyEventListener` implemente <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents>:
...
6. En la misma clase, agregue otro controlador de eventos para el evento DTE <xref:EnvDTE.ProjectItemsEventsClass.ItemRenamed>, que se produce cada vez que se cambia el nombre de un elemento de proyecto.
...
10. Llame a este método después de que todo el resto del código de `ShowMessageBox` para modificar el nombre de archivo del elemento en el proyecto compartido. Inserte esto después del código que obtiene la ruta de acceso completa del elemento en el proyecto compartido.
...
11. Compile y ejecute el proyecto. Cree una C# aplicación de Hub universal en la instancia experimental, vaya al menú **herramientas** y haga clic en **invocar TestUniversalProject**y compruebe el texto en el panel de salida general. El nombre del primer elemento del proyecto compartido (esperamos que sea el archivo *app. Xaml* ) debe cambiarse y debería ver que se ha desencadenado el evento <xref:EnvDTE.ProjectItemsEventsClass.ItemRenamed>. En este caso, puesto que el cambio de nombre de *app. Xaml* hace que también se cambie el nombre de *app.Xaml.CS* , debería ver cuatro eventos (dos para cada proyecto de plataforma). (Los eventos DTE no realizan el seguimiento de los elementos del proyecto compartido). Debería ver dos eventos de <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemDeleted%2A> (uno para cada uno de los proyectos de plataforma), pero no los eventos de <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemAdded%2A>.
...
13. Compile y ejecute el proyecto. Cree un C# proyecto universal en la instancia experimental, vaya al menú **herramientas** y haga clic en **invocar TestUniversalProject**y compruebe el texto en el panel de salida general. Una vez cambiado el nombre del archivo en el proyecto de la plataforma, debería ver un evento <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemAdded%2A> y un evento <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemDeleted%2A>. Puesto que no se ha podido cambiar el archivo porque no se propagan otros archivos y, como los cambios en los elementos de un proyecto de plataforma, no se propagan en ningún lugar, solo hay uno de estos eventos.
