...
| MSBuild consume automáticamente los archivos *.js* o *.css.* | Y | En **el Explorador**de soluciones , puede expandir el nodo de referencia del SDK de JavaScript para mostrar archivos *.js* o *.css* individuales y, a continuación, generar `<source include/>` etiquetas arrastrando esos archivos a sus archivos de origen. El SDK es compatible con F5 y con la instalación de paquetes automática. | Y | |
