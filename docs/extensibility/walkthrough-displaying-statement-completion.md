...
Puede implementar la finalización de instrucciones basada en el lenguaje si define los identificadores para los que desea proporcionar la finalización y, a continuación, desencadenar una sesión de finalización. Puede definir la finalización de instrucciones en el contexto de un servicio de lenguaje, definir su propia extensión de nombre de archivo y tipo de contenido y, a continuación, Mostrar la finalización solo para ese tipo. O bien, puede desencadenar la finalización de un tipo de contenido existente (por ejemplo, "Plaintext"). En este tutorial se muestra cómo desencadenar la finalización de instrucciones para el tipo de contenido "Plaintext", que es el tipo de contenido de los archivos de texto. El tipo de contenido "texto" es el antecesor de todos los demás tipos de contenido, incluidos los archivos de código y XML.
 ...
 La finalización de instrucciones se desencadena normalmente escribiendo ciertos caracteres (por ejemplo, al escribir el principio de un identificador como "Using"). Normalmente se descarta presionando la **barra espaciadora**, la **tabulación**o la tecla **entrar** para confirmar una selección. Puede implementar las características de IntelliSense que se desencadenan al escribir un carácter mediante un controlador de comandos para las pulsaciones de teclas (la interfaz <xref:Microsoft.VisualStudio.OLE.Interop.IOleCommandTarget>) y un proveedor de controladores que implementa la interfaz <xref:Microsoft.VisualStudio.Editor.IVsTextViewCreationListener>. Para crear el origen de finalización, que es la lista de identificadores que participan en la finalización, implemente la interfaz <xref:Microsoft.VisualStudio.Language.Intellisense.ICompletionSource> y un proveedor de origen de finalización (la interfaz <xref:Microsoft.VisualStudio.Language.Intellisense.ICompletionSourceProvider>). Los proveedores son partes de componentes de Managed Extensibility Framework (MEF). Son responsables de exportar las clases de origen y de controlador e importar los servicios y los agentes, por ejemplo, el <xref:Microsoft.VisualStudio.Text.Operations.ITextStructureNavigatorSelectorService>, que permite la navegación en el búfer de texto y el <xref:Microsoft.VisualStudio.Language.Intellisense.ICompletionBroker>, que desencadena la sesión de finalización.
...
1. Cree un C# proyecto de VSIX. (En el cuadro de diálogo **nuevo proyecto** , seleccione **Visual C# /extensibilidad**, **Proyecto VSIX**). Asigne a la solución el nombre `CompletionTest`.
...
4. Agregue campos privados para el proveedor de origen, el búfer de texto y una lista de objetos de <xref:Microsoft.VisualStudio.Language.Intellisense.Completion> (que se corresponden con los identificadores que participarán en la sesión de finalización):

     [!code-csharp[VSSDKCompletionTest#3](../extensibility/codesnippet/CSharp/walkthrough-displaying-statement-completion_3.cs)]
     [!code-vb[VSSDKCompletionTest#3](../extensibility/codesnippet/VisualBasic/walkthrough-displaying-statement-completion_3.vb)]

5. Agregue un constructor que establezca el proveedor y el búfer de origen. La clase `TestCompletionSourceProvider` se define en pasos posteriores:

     [!code-csharp[VSSDKCompletionTest#4](../extensibility/codesnippet/CSharp/walkthrough-displaying-statement-completion_4.cs)]
     [!code-vb[VSSDKCompletionTest#4](../extensibility/codesnippet/VisualBasic/walkthrough-displaying-statement-completion_4.vb)]

6. Implemente el método <xref:Microsoft.VisualStudio.Language.Intellisense.ICompletionSource.AugmentCompletionSession%2A> agregando un conjunto de finalizaciones que contiene las finalizaciones que desea proporcionar en el contexto. Cada conjunto de finalización contiene un conjunto de <xref:Microsoft.VisualStudio.Language.Intellisense.Completion> finalizaciones y corresponde a una pestaña de la ventana de finalización. (En proyectos de Visual Basic, las pestañas de la ventana de finalización se denominan **común** y **todos**). El método `FindTokenSpanAtPosition` se define en el paso siguiente.
...
8. Implemente el método `Dispose()`:
...
3. Implemente el método <xref:Microsoft.VisualStudio.Language.Intellisense.ICompletionSourceProvider.TryCreateCompletionSource%2A> para crear una instancia del origen de finalización.
...
5. Implemente el método <xref:Microsoft.VisualStudio.Editor.IVsTextViewCreationListener.VsTextViewCreated%2A> para crear una instancia del controlador de comandos.
...
## <a name="implement-the-completion-command-handler"></a>Implementar el controlador de comandos de finalización
 Como la finalización de instrucciones se desencadena con pulsaciones de teclas, debe implementar la interfaz <xref:Microsoft.VisualStudio.OLE.Interop.IOleCommandTarget> para recibir y procesar las pulsaciones de teclas que desencadenan, confirman y descartan la sesión de finalización.
...
4. Implemente el método <xref:Microsoft.VisualStudio.OLE.Interop.IOleCommandTarget.QueryStatus%2A> pasando el comando a lo largo de:
...
5. Implemente el método <xref:Microsoft.VisualStudio.OLE.Interop.IOleCommandTarget.Exec%2A>. Cuando este método recibe una pulsación de tecla, debe realizar una de estas acciones:
   ...
   - Permite pasar el comando al siguiente controlador. (El resto de comandos).
     ...
     Dado que este método puede mostrar la interfaz de usuario, llame a <xref:Microsoft.VisualStudio.Shell.VsShellUtilities.IsInAutomationFunction%2A> para asegurarse de que no se llama en un contexto de automatización:
...
7. El ejemplo siguiente es un método privado que cancela la suscripción del evento <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseSession.Dismissed>:
