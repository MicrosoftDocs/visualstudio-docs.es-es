...
1. Si va a usar C#, busque la línea `<Window x:Class="ExpenseIt.MainWindow"` y sustitúyala por `<NavigationWindow x:Class="ExpenseIt.MainWindow"`.
     ...
     Tenga en cuenta que al cambiar la etiqueta `<Window` a `<NavigationWindow`, IntelliSense también cambiará automáticamente la etiqueta de cierre a `</NavigationWindow>`.
