---
title: Navegar y actualizar un modelo en el código del programa | Microsoft Docs
ms.date: 11/15/2016
...
## <a name="in-this-topic"></a>En este tema
 [Definición de DSL de ejemplo](#example)

 [Navegar por el modelo](#navigation)

 [Obtener acceso a la información de clase](#metadata)

 [Realizar cambios dentro de una transacción](#transaction)

 [Crear elementos de modelo](#elements)
 ...
 [Navegar entre formas y elementos](#views)

 [Propiedades de las formas y los conectores](#shapeProperties)

 [DocView y noncdata](#docdata)

 En un tema independiente se describen las formas, los conectores y los diagramas, y sus relaciones con los elementos del modelo. Para obtener más información, consulte [Cómo: navegar y actualizar un diagrama](../misc/how-to-navigate-and-update-a-diagram.md).
 ...
 **ElementLinks**. Una relación también se representa mediante un elemento de modelo denominado *vínculo*, que es una instancia del tipo de relación de dominio. Un vínculo siempre tiene un elemento de origen y un elemento de destino. El elemento de origen y el elemento de destino pueden ser iguales.
 ...
 De forma predeterminada, no se permite que más de una instancia de una relación vincule ningún par de elementos de modelo. Pero si está en la definición de DSL, la `Allow Duplicates` marca es true para la relación, puede haber más de un vínculo y debe usar `GetLinks` :
...
## <a name="deleting-elements"></a><a name="deleteelements"></a> Eliminar elementos
...
- Elementos de los roles para los que la `PropagatesDelete` marca es true. Por ejemplo, se eliminará la forma que muestra el elemento.
  ...
  De forma predeterminada, todas las relaciones de incrustación tienen `PropagatesDelete` el valor true en el rol de destino. Al eliminar `henry` , no se elimina el `familyTree` , sino que se `familyTree.Delete()` eliminan todos los `Persons` . Para obtener más información, vea [personalizar el comportamiento de eliminación](../modeling/customizing-deletion-behavior.md).
...
### <a name="presentation-elements"></a>Elementos de presentación
 ![Diagrama de clases de forma base y tipos de elementos](../modeling/media/dslshapesandelements.png "DSLshapesAndElements")
...
## <a name="see-also"></a>Consulte también
 <xref:Microsoft.VisualStudio.Modeling.ModelElement>[Validación en un lenguaje específico de dominio que](../modeling/validation-in-a-domain-specific-language.md) [genera código a partir de un lenguaje específico de dominio](../modeling/generating-code-from-a-domain-specific-language.md) [Cómo: usar transacciones para actualizar el modelo](../modeling/how-to-use-transactions-to-update-the-model.md) [integrar modelos mediante el uso de Modelbus de Visual Studio](../modeling/integrating-models-by-using-visual-studio-modelbus.md) [responder y propagar los cambios](../modeling/responding-to-and-propagating-changes.md)
