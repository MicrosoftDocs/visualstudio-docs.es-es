...
## <a name="unmanaged-code"></a>Código no administrado  
 En este ejemplo se muestra una implementación de `IDebugProperty2::GetPropertyInfo` para la variable local de un método en código no administrado. También se muestran dos funciones auxiliares, `FieldGetType` `FieldGetValue` que se usan para obtener el tipo y el valor del campo, respectivamente. Tenga en cuenta que `VARIANT` se usan para el valor y el tipo del campo, ya que `VARIANT` puede controlar una amplia variedad de tipos de valor. En esta implementación, `FieldGetValue` devuelve un objeto [IDebugField](../../extensibility/debugger/reference/idebugfield.md) que se convierte posteriormente en un valor de una llamada a `FieldGetPrimitiveValue` (que se muestra en la [evaluación de variables locales](../../extensibility/debugger/evaluating-locals.md)).  
