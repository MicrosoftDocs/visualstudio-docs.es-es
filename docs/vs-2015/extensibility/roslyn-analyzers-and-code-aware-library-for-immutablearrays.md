...
La creación de matrices vacías para rellenar con líneas de código posteriores y el uso de la sintaxis del inicializador de colección son muy familiares para los desarrolladores de C. Sin embargo, escribir el mismo código para un ImmutableArray se bloquea en tiempo de ejecución:
...
## <a name="finding-relevant-syntax-node-types-to-trigger-your-analyzer"></a>Búsqueda de tipos de nodos de sintaxis relevantes para activar el analizador
Para comenzar a compilar el analizador, primero averiguar qué tipo de SyntaxNode necesita buscar.   Inicie el visualizador de sintaxis desde el menú Ver &#124; otro &#124; visualizador de **sintaxis Roslyn**.
...
## <a name="creating-the-analyzer-project"></a>Creación del proyecto Analyzer
...
## <a name="initalizing-the-analyzer"></a>Initalizar el analizador
...
## <a name="setting-properties-for-users-of-your-analyzer"></a>Configuración de propiedades para los usuarios del analizador
...
### <a name="launching-visual-studio-with-your-analyzer-the-first-time"></a>Iniciar Visual Studio con el analizador por primera vez
...
Todavía verá ondulaciones `ImmutableArray`rojas debajo, así que coloque el símbolo de intercalación en ese identificador y presione **CTRL+.** (período) para abrir el menú de arreglos `using` sugerido y elegir agregar la instrucción apropiada.

**Guardar todo y cerrar** la segunda instancia de Visual Studio por ahora para poner en un estado limpio para continuar.

## <a name="finishing-the-analyzer-using-edit-and-continue"></a>Finalización del analizador mediante Editar y continuar
...
**Obtener ImmutableArray\<T> objeto Type.** Debe comprobar si el tipo que se está creando es ImmutableArray. En primer lugar, obtendrá el objeto que representa este tipo. Compruebe los tipos mediante el modelo semántico para asegurarse de que tiene exactamente el tipo correcto y no se compara la cadena de ToString(). Introduzca la siguiente línea de código al final de la función:
...
Los tipos genéricos se designan en metadatos con comillas inversas (') y el número de parámetros genéricos. Es por eso que no ves "... ImmutableArray\<T>" en el nombre de los metadatos.
...
**Stub out miembros derivados.** Ahora, coloque el símbolo de intercalación del editor en el identificador `CodeFixProvider` y presione **CTRL+.** (período) para eliminar la implementación de esta clase base abstracta. Esto genera una propiedad y un método para usted.
...
**Busque el nodo con el problema.** Se pasa en el intervalo del contexto, pero el nodo que encuentra puede no ser el código que tiene que cambiar. El diagnóstico notificado solo proporcionaba el intervalo para el identificador de tipo (donde pertenecía el squiggle), pero debe reemplazar toda la expresión de creación de objetos, incluido el `new` keywoard al principio y los paréntesis al final. Agregue el código siguiente al método (y use **CTRL+.** para añadir `using` una `ObjectCreationExpressionSyntax`declaración para ):
...
Debe colocar el símbolo de intercalación del `CodeAction`editor en el identificador y, a continuación, usar **CTRL+.** (período) para agregar `using` la declaración adecuada para este tipo.

A continuación, coloque el símbolo `ChangeToImmutableArrayEmpty` de intercalación del editor en el identificador y use **CTRL+.** de nuevo para generar este código auxiliar de método para usted.
...
Deberá colocar el símbolo de intercalación `SyntaxGenerator` del editor en el identificador y usar **CTRL+.** (período) para agregar `using` la declaración adecuada para este tipo.

Este código `SyntaxGenerator`usa , que es un tipo muy útil para construir código nuevo. Después de obtener un generador para `ChangeToImmutableArrayEmpty` el `MemberAccessExpression`documento que tiene el problema de código, llama , pasando el tipo que tiene el miembro al que queremos tener acceso y pasando el nombre del miembro como una cadena.
...
[\\Conversación de 2015](https://channel9.msdn.com/events/Build/2015/3-725)
...
Otros documentos en las reglas de FxCop del sitio 
[de GitHub OSS](https://github.com/dotnet/roslyn/tree/master/docs/analyzers)[implementadas con analizadores Roslyn en GitHub](https://github.com/dotnet/roslyn/tree/master/src/Features/Core/Portable/Diagnostics/Analyzers)
