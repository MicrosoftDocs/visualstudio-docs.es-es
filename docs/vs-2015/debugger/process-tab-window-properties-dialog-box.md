---
title: Procesar la ficha, cuadro de diálogo de propiedades de ventana | Documentos de Microsoft
ms.custom: ''
ms.date: 2018-06-30
ms.prod: visual-studio-dev14
ms.reviewer: ''
ms.suite: ''
ms.technology:
- vs-ide-debug
ms.tgt_pltfrm: ''
ms.topic: article
helpviewer_keywords:
- Window Properties dialog box, Process Tab
ms.assetid: bb464429-4e7c-4919-98b5-1aaafe89bb5f
caps.latest.revision: 7
author: mikejo5000
ms.author: mikejo
manager: ghogen
ms.openlocfilehash: 3cc1a02108708b3a9430af50d0b69b1ed1610c1b
ms.sourcegitcommit: 55f7ce2d5d2e458e35c45787f1935b237ee5c9f8
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 08/22/2018
ms.locfileid: "47567859"
---
# <a name="process-tab-window-properties-dialog-box"></a>Pestaña Proceso (Cuadro de diálogo Propiedades de la ventana)
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

La versión más reciente de este tema puede encontrarse en [pestaña proceso, el cuadro de diálogo de propiedades de ventana](https://docs.microsoft.com/visualstudio/debugger/process-tab-window-properties-dialog-box).  
  
Use la **proceso** ficha para identificar el proceso de la ventana seleccionada. Para mostrar el [ventana cuadro de diálogo de propiedades](../debugger/window-properties-dialog-box.md), mover el foco a la [Windows Vista](../debugger/windows-view.md) ventana. Seleccione el nodo de una ventana en el árbol y luego elija **propiedades** desde el **vista** menú.  
  
 Las siguientes opciones están disponibles en el **proceso** pestaña:  
  
|Entrada|Descripción|  
|-----------|-----------------|  
|**Identificador del proceso**|El identificador de proceso que posee el subproceso que creó esta ventana. Elija este valor para ver las propiedades de este proceso.|  
|**Identificador de subproceso**|El identificador del subproceso que creó esta ventana. Elija este valor para ver las propiedades de este subproceso.|



