---
title: 'Cómo: usar la ventana subprocesos | Microsoft Docs'
ms.date: 11/15/2016
...
En la ventana **subprocesos** , puede examinar y trabajar con los subprocesos de la aplicación que está depurando.  
  
 La ventana **subprocesos** contiene una tabla en la que cada fila representa un subproceso de la aplicación. De forma predeterminada, la tabla hace una lista de todos los subprocesos, pero puede filtrar la lista para mostrar solo los que le interesan. Cada columna contiene un tipo diferente de información. También puede ocultar algunas columnas. Si muestra todas las columnas, la siguiente información aparece de izquierda a derecha:  
  
- La columna de marcas, donde puede marcar un subproceso al que desea prestar atención especial. Para obtener información sobre cómo marcar un subproceso, consulte [Cómo: marcar y desmarcar subprocesos](../debugger/how-to-flag-and-unflag-threads.md).  
...
- La columna de **identificadores administrados** , que contiene los números de identificación administrados para los subprocesos administrados.  
  
- La columna **categoría** , que clasifica los subprocesos como subprocesos de la interfaz de usuario, controladores de llamadas a procedimientos remotos o subprocesos de trabajo. Una categoría especial identifica el subproceso principal de la aplicación.  
  
- La columna de **nombre** , que identifica cada subproceso por nombre, si tiene uno, o como \<No Name> .  
  
- La columna **Location** , que muestra dónde se está ejecutando el subproceso. Puede expandir esta ubicación para mostrar la pila de llamadas completa del subproceso.  
  
- La columna **Priority** , que contiene la prioridad o la prioridad que el sistema ha asignado a cada subproceso.  
  
- La columna **affinity mask** , que es una columna avanzada que suele estar oculta. Esta columna muestra la máscara de afinidad del procesador para cada subproceso. En un sistema multiprocesador, la máscara de afinidad determina los procesadores en los que se puede ejecutar un subproceso.  
  
- La columna **recuento de suspensión** , que contiene el recuento de suspendidos. Este recuento determina si un subproceso se puede ejecutar. Para obtener una explicación sobre el recuento de suspensión, vea la sección Inmovilizar y reanudar subprocesos más adelante en este tema.  
...
- En el menú **depurar** , seleccione **ventanas**y, a continuación, haga clic en **subprocesos**.  
  
### <a name="to-display-or-hide-a-column"></a>Mostrar u ocultar columnas  
  
- En la barra de herramientas de la parte superior de la ventana **subprocesos** , haga clic en **columnas**y, a continuación, Active o desactive el nombre de la columna que desea mostrar u ocultar.  
...
- Realice uno de estos pasos:  
  ...
  - Haga clic con el botón secundario en un subproceso y haga clic en **cambiar a subproceso**.  
...
1. En la barra de herramientas de la parte superior de la ventana **subprocesos** , haga clic en el botón situado en la parte superior de cualquier columna.  
...
- En la barra de herramientas de la ventana **subprocesos** , haga clic en la lista **Agrupar por** y, a continuación, en los criterios por los que desea agrupar los subprocesos.  
  
#### <a name="to-sort-threads-within-groups"></a>Para ordenar los subprocesos dentro de los grupos  
  
1. En la barra de herramientas de la parte superior de la ventana **subprocesos** , haga clic en la lista **Agrupar por** y, a continuación, haga clic en los criterios por los que desea agrupar los subprocesos.  
...
## <a name="searching-for-specific-threads"></a>Buscar Subprocesos concretos  
 En [!INCLUDE[vs_dev11_long](../includes/vs-dev11-long-md.md)], puede buscar subprocesos que coincidan con una cadena especificada. Al buscar subprocesos en la ventana **subprocesos** , la ventana muestra todos los subprocesos que coinciden con la cadena de búsqueda en cualquier columna. Esa información incluye la ubicación del subproceso que aparece en la parte superior de la pila de llamadas en la columna **Ubicación** . Sin embargo, no se busca en la pila de llamadas completa de forma predeterminada.  
...
## <a name="freezing-and-thawing-threads"></a>Inmovilizar y reanudar subprocesos  
 ...
 En código nativo, puede suspender o reanudar subprocesos llamando a las funciones de Windows `SuspendThread` y `ResumeThread` o a las funciones de MFC [CWinThread:: SuspendThread](https://msdn.microsoft.com/library/57189c7e-fd71-42e5-bc4b-3de7cd373d28) y [CWinThread:: ResumeThread](https://msdn.microsoft.com/library/d6f97a2f-5c9f-4ee1-b978-d74938784db5). Si llama a `SuspendThread` o `ResumeThread` , cambia el *recuento de suspensión*, que aparece en la ventana **subprocesos** . Sin embargo, si inmoviliza o reanuda un subproceso nativo, no cambia el recuento de suspendidos. En código nativo, un subproceso no se puede ejecutar a menos que se reanude y tenga un recuento de suspensión de cero.  
...
- Elija el botón de marca situado en la esquina superior izquierda de la ventana **subprocesos** .  
