...
El simulador de Visual Studio para las aplicaciones de la Tienda Windows es una aplicación de escritorio que simula una aplicación de la Tienda Windows. Puede ejecutar aplicaciones y simular eventos táctiles y rotaciones comunes en su equipo de desarrollo. También puede elegir el tamaño físico de la pantalla y la resolución que quiera emular y simular las propiedades de conexión de red.  
 ...
 El simulador de Visual Studio para aplicaciones de la Tienda Windows no se ejecuta en un entorno aislado de la máquina local. Por lo tanto, los errores que ocurren en el simulador, como un error irrecuperable del sistema, también pueden afectar a toda la máquina.  
...
- ![Botón de modo del mouse](../debugger/media/simulator-mousemodebtn.png "SIMULATOR_MouseModeBtn") Modo de mouse: establece el modo de interacción en los gestos del mouse. Los gestos del mouse son los clics, los dobles clics y arrastrar.  
...
- ![Botón iniciar emulación táctil](../debugger/media/simulator-starttouchemulationbtn.png "SIMULATOR_StartTouchEmulationBtn") Iniciar emulación táctil: establece el modo de interacción en gestos táctiles de un solo dedo. Los eventos de un solo dedo son pulsar, arrastrar y deslizar rápidamente.  
     ...
     ![Objetivo del simulador de un dedo](../debugger/media/simulator-onefinger.png "SIMULATOR_OneFinger") El icono de destino único indica la ubicación de los eventos en el simulador. Use el mouse para colocar el puntero.  
  
     ![Destino táctil de un dedo](../debugger/media/simulator-onefingerengaged.png "SIMULATOR_OneFingerEngaged") Presione el botón primario del mouse para activar el modo táctil. Por ejemplo, haga clic en el botón para simular una pulsación, o manténgalo presionado para arrastrar o deslizar rápidamente.  
...
- ![Destino de dos dedos del simulador](../debugger/media/simulator-twofinger.png "SIMULATOR_TwoFinger")  
  ...
  - Mueva el mouse para colocar los iconos sobre el objeto en la pantalla del dispositivo.  

  - Gire la rueda del mouse hacia atrás o hacia delante para cambiar la distancia simulada de los dos dedos antes de realizar la emulación táctil de acercar o alejar.  

- ![Desplazar, zoom y girar destinos](../debugger/media/simulator-twofingerengaged.png "SIMULATOR_TwoFingerEngaged")  

  - Presione el botón primario y gire la rueda hacia atrás (hacia ti) para acercar (acercar los dedos).  

  - Presione el botón primario y gire la rueda del mouse hacia delante (hacia el lado contrario a donde tú estás) para alejar (separar los dedos).  
...
- Mueva el mouse para colocar los iconos sobre el objeto en la pantalla del dispositivo.  
  ...
  - Gire la rueda del mouse hacia atrás o hacia delante para cambiar la orientación simulada de los dos dedos antes de rotar el objeto.  

- Presione el botón primario y gire la rueda hacia atrás (hacia ti) para rotar el objeto hacia la izquierda. Al girar la rueda del mouse, uno de los dos iconos de destino rota alrededor del otro para indicar el tamaño relativo de la rotación.  

  - Presione el botón primario y gire la rueda del mouse hacia delante (hacia el lado contrario a donde te encuentras) para rotar el objeto en el sentido de las agujas del reloj.  

## <a name="BKMK_Enable_or_disable_Always_on_top_mode"></a> Habilitar o deshabilitar el modo Siempre visible  
 Puede configurar la ventana del simulador para que siempre esté encima de las demás. El botón **Alternar la ventana de nivel superior** habilita o deshabilita el modo **Siempre visible** de la ventana del simulador.  
  
## <a name="BKMK_Change_the_device_orientation"></a> Cambiar la orientación del dispositivo  
 Puede cambiar la orientación del dispositivo entre vertical y horizontal rotándolo 90 grados en cualquier dirección.  
  
> [!NOTE]
> El simulador no respeta la propiedad [DisplayProperties.AutoRotationPreferences](https://msdn.microsoft.com/library/windows/apps/windows.graphics.display.displayproperties.autorotationpreferences.aspx) de un proyecto. Por ejemplo, si el proyecto establece la orientación en `Landscape`y, después, tú rotes el simulador para situarlo en orientación vertical, la imagen de la pantalla del simulador también se rotará y cambiará de tamaño. Pruebe la configuración en un dispositivo real.  
  
> [!NOTE]
> Si rota el simulador de modo que su borde sea mayor que la pantalla en la que aparece, el tamaño cambia para ajustarse automáticamente a la pantalla. Si lo vuelve a rotar, no recupera su tamaño original.  
...
> [!NOTE]
> Puede guardar versiones a escala de las imágenes de mapa de bits en la aplicación y Windows cargará la imagen correcta para la escala actual. Para más información, vea [Diseño dinámico 101](https://msdn.microsoft.com/library/windows/apps/dn958435.aspx). Sin embargo, si cambia la resolución del simulador de modo que Windows elija otra imagen para ajustarla a la resolución, deberá detener y reiniciar la sesión de depuración para ver la nueva imagen.  
...
> [!NOTE]
> La captura de pantalla se guarda en la resolución actual del simulador. Para cambiar la resolución, elige el botón **Cambiar resolución** .  
...
## <a name="BKMK_Simulate_network_connection_properties"></a> Simular propiedades de conexión de red  
 ...
 Para simular las propiedades de red:  
...
2. En el cuadro de diálogo **Establecer propiedades de red** , seleccione **Usar propiedades de red simuladas**.  
    ...
    Desactive la casilla de verificación para quitar la simulación y volver a las propiedades de red de la interfaz conectada actualmente.  
