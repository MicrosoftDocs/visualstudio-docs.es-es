...
## <a name="allowing-projects-to-load-in-multiple-versions"></a>Permitir cargar proyectos en varias versiones  
 ...
 Antes de carga un proyecto, Visual Studio llama el <xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgradeViaFactory4.UpgradeProject_CheckOnly%2A> método para determinar si se puede actualizar el proyecto. Si se puede actualizar el proyecto, el `UpgradeProject_CheckOnly` método establece una marca que provoca una llamada posterior a la <xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectUpgradeViaFactory.UpgradeProject%2A> método para actualizar el proyecto. Dado que no se pueden actualizar proyectos incompatibles, `UpgradeProject_CheckOnly` comprueba primero la compatibilidad del proyecto, como se describe en la sección anterior.  
...
4.  `VSPUVF_PROJECT_ONEWAYUPGRADE`: Hace que el proyecto incompatible con una versión anterior.  
...
##  <a name="BKMK_Incompat"></a> Marcar un proyecto como Incompatible  
 Puede marcar un proyecto como incompatible con versiones anteriores de Visual Studio.  Por ejemplo, suponga que crea un proyecto que usa una característica de .NET Framework 4.5. Dado que no puede compilarse este proyecto [!INCLUDE[vs_dev10_long](../includes/vs-dev10-long-md.md)], puede marcarlo como incompatible para impedir que esa versión intente cargarlo.  
  
 El componente que agrega la característica incompatible es responsable de marcar el proyecto como incompatible. El componente debe tener acceso a la <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchy> interfaz que representa los proyectos de interés.  
...
>  Puede almacenar en caché el resultado de la comprobación de compatibilidad mediante el método `UpgradeProject_CheckOnly` para que se pueda usar también por la llamada subsiguiente a `CreateProject`.  
 ...
 Por ejemplo, si la `UpgradeProject_CheckOnly` y `CreateProject` métodos que están escritos para un [!INCLUDE[vs_dev10_long](../includes/vs-dev10-long-md.md)] con el sistema de proyectos de SP1 examinar un archivo de proyecto y observan que la `<MinimumVisualStudioVersion>` propiedad de compilación es "11.0", Visual Studio 2010 con SP1 no cargará el proyecto. Además, el **navegador de soluciones** indicará que el proyecto es "incompatible" y no lo cargará.  
