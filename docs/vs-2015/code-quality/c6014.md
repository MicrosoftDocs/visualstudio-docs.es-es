...
```  
 ...
 Tenga en cuenta que el análisis de código no reconocerá la implementación real de un asignador de memoria (que implique la aritmética de direcciones) y no reconocerá que la memoria está asignada (aunque se reconozcan muchos contenedores). En este caso, el analizador no reconoce que la memoria se asignó y emite esta advertencia. Para suprimir el falso positivo, use una `#pragma` Directiva en la línea que preceda a la llave `{` de apertura del cuerpo de la función.  
