...
Un archivo del proyecto puede contener uno o vario elementos `Target` que definen cómo se compila el proyecto. El [!INCLUDE[vstecmsbuildengine](../includes/vstecmsbuildengine-md.md)] motor de ( [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] ) compila el primer proyecto que encuentra y las dependencias, a menos que el archivo de proyecto contenga un `DefaultTargets` atributo, un `InitialTargets` atributo o un destino se especifique en la línea de comandos mediante el modificador **/target** .  
...
## <a name="using-the-target-switch"></a>Usar el modificador /target  
 Si un destino predeterminado no está definido en el archivo del proyecto, o si no desea usar ese destino predeterminado, puede usar el modificador de la línea de comandos **/target** para especificar un destino diferente. Los destinos especificados con el modificador **/target** se ejecutan en lugar de los destinos especificados por el `DefaultTargets` atributo. Los destinos especificados en el atributo `InitialTargets` siempre se ejecutan primero.  
...
- Especifique el destino como primer destino mediante el modificador de línea de comandos **/target** . Por ejemplo:  
...
- Enumere los destinos, separados por punto y coma o comas, mediante el modificador de la línea de comandos **/target** . Por ejemplo:  
