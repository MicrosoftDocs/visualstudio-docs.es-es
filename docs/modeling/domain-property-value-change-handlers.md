...
description: Obtenga información sobre los controladores de cambio de valores de propiedad de dominio que se pueden usar en un Visual Studio específico del dominio.
...
ms.locfileid: "112389002"
---
# <a name="domain-property-value-change-handlers"></a>Controladores de cambio de valores de propiedad de dominio

En un Visual Studio específico del dominio, cuando cambia el valor de una propiedad de dominio, los métodos y se `OnValueChanging()` `OnValueChanged()` invocan en el controlador de propiedades de dominio. Para responder al cambio, puede invalidar estos métodos.

## <a name="override-the-property-handler-methods"></a>Invalidar los métodos del controlador de propiedades
...
Por ejemplo, supongamos que tiene una clase de dominio denominada que tiene una propiedad de dominio de `Comment` cadena denominada y una propiedad de entero denominada `Text` `TextLengthCount` . Para que siempre contenga la longitud de la cadena, podría escribir el código siguiente en un `TextLengthCount` archivo independiente en el proyecto `Text` dsl:
...
### <a name="cancel-a-change"></a>Cancelación de un cambio
...
En lugar del ejemplo anterior, podría establecer el campo **Kind** de para `TextLengthCount` que se **calcule en la** definición de DSL. Proporcionaría su propio **método Get** para esta propiedad de dominio. El **método Get** devolvería la longitud actual de la `Text` cadena.
...
Si define changeRule, se ejecuta al final de una transacción en la que cambia el valor de una propiedad.  Para obtener más información, vea [Reglas propagar cambios dentro del modelo](../modeling/rules-propagate-changes-within-the-model.md).

Si se realizan varios cambios en una transacción, la regla de cambio se ejecuta cuando se completan todos. Por el contrario, OnValue... Los métodos se ejecutan cuando no se han realizado algunos de los cambios. Según lo que quiera lograr, una regla de cambio podría ser más apropiada.

También puede usar changeRule para ajustar el nuevo valor de la propiedad para mantenerlo dentro de un intervalo específico.
