...
helpviewer_keywords: 
  - "Lenguaje específico de dominio, eliminación"
...
Normalmente, al eliminar un elemento también se eliminan los elementos relacionados.  Se eliminan todas las relaciones conectadas a él y todos los elementos secundarios.  Este comportamiento se llama *propagación de la eliminación*.  Puede personalizar la propagación de la eliminación, por ejemplo, para organizar que se eliminen otros elementos relacionados.  Escribiendo código de programa puede hacer que la propagación de la eliminación dependa del estado del modelo.  También puede hacer que se produzcan otros cambios en respuesta a una eliminación.  
...
-   [Invalidar el cierre de la eliminación](#closure): use esta técnica cuando la eliminación pueda provocar la eliminación de los elementos vecinos.  
  
-   [Usar OnDeleting y OnDeleted](#ondeleting): use estos métodos cuando la respuesta pueda incluir otras acciones tales como actualizar un valor dentro o fuera del almacén.  
  
-   [Reglas de eliminación](#rules): use reglas para propagar actualizaciones de cualquier tipo dentro del almacén, cuando un cambio podría producir otros.  
  
-   [Eventos de eliminación](#rules): use los eventos del almacén para propagar las actualizaciones fuera del almacén, por ejemplo, a otros documentos de [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)].  
  
-   [UnMerge](#unmerge): use la operación UnMerge para deshacer la operación de combinación que asoció un elemento secundario a su primario.  
  
##  <a name="default"></a> Comportamiento predeterminado de la eliminación  
 De forma predeterminada, la propagación de la eliminación cumple estas reglas:  
  
-   Si un elemento se elimina, todos los elementos incrustados también se eliminan.  Los elementos incrustados son los destinos de relación de incrustación para las cuales este elemento es el origen.  Por ejemplo, si hay una relación de incrustación de Album a Song, cuando se elimina un Album determinado, todas sus Songs se eliminarán también.  
  
     En cambio, al eliminar una canción no se elimina el álbum.  
  
-   De forma predeterminada, la eliminación no se propaga por las relaciones de referencia.  Si hay una relación de referencia ArtistPlaysOnAlbum de Album a Artist, al eliminar un álbum no se elimina el artista relacionado y al eliminar un artista no se elimina ningún álbum.  
...
##  <a name="property"></a> Establecer la opción Propagate Delete \(Propagar eliminación\) de un rol  
...
#### Para establecer la propagación de la eliminación  
  
1.  En el diagrama DSL Definition \(Definición de DSL\), seleccione el *rol* al que quiera propagar la eliminación.  El rol está representado por la línea situada a la izquierda o derecha de un cuadro de relación de dominio.  
...
2.  En la ventana Properties \(Propiedades\), establezca la propiedad **Propagates Delete** \(Propagar eliminación\).  
...
3.  Presione F5 y compruebe que:  
 ...
 También puede ver la opción **Propagates Delete** \(Propagar eliminación\) en la ventana **DSL Details** \(Detalles de DSL\).  Seleccione una clase de dominio y, en la ventana DSL Details \(Detalles de DSL\), abra la página **Delete Behavior** \(Comportamiento de eliminación\) haciendo clic en el botón situado en un lado de la ventana.  La opción **Propagate** \(Propagar\) se muestra para el rol opuesto de cada relación.  La columna **Delete Style** \(Estilo de eliminación\) indica si el valor de la opción **Propagate** \(Propagar\) es el predeterminado pero no tiene ningún efecto diferente.  
  
## Propagación de la eliminación mediante código de programa  
...
> [!NOTE]
>  Para agregar código de programa a su definición de DSL, cree un archivo de código diferente en el proyecto **Dsl** y escriba definiciones parciales para aumentar las clases en la carpeta Generated Code.  Para obtener más información, vea [Escribir código para personalizar un lenguaje específico de dominio](../modeling/writing-code-to-customise-a-domain-specific-language.md).  
  
##  <a name="closure"></a> Definir un cierre de eliminación  
 La operación de eliminación usa la clase *suModelo***DeleteClosure** para determinar qué elementos eliminará, dada una selección inicial.  Llama a `ShouldVisitRelationship()` y a `ShouldVisitRolePlayer()` repetidamente, recorriendo el gráfico de relaciones.  Puede invalidar estos métodos.  ShouldVisitRolePlayer incluye la identidad de un vínculo y el elemento en uno de los roles del vínculo.  Debe devolver uno de los siguientes valores:  
  
-   **VisitorFilterResult.Yes**: el elemento se debe eliminar y el rastreador debe continuar para probar otros vínculos del elemento.  
  
-   **VisitorFilterResult.DoNotCare**: el elemento no se debe eliminar a menos que otra consulta responda que debe eliminarse.  
  
-   **VisitorFilterResult.Never**: el elemento no se debe eliminar, aunque otra consulta responda **Yes** y el rastreador no debe probar otros vínculos del elemento.  
...
```  
 ...
 Sin embargo, en esta técnica se da por hecho que la eliminación afecta solo a los vecinos en el gráfico de relaciones: no puede usar este método para eliminar un elemento que está en otra parte del modelo.  No puede usarlo si quiere agregar elementos o realizar otros cambios en respuesta a una eliminación.  
...
1.  Las reglas **Deleting** y **Delete** se desencadenan solo en una transacción y no en Undo o Redo.  Puede establecerlas para que se pongan en la cola de ejecución al final de la transacción en la que se realiza la eliminación.  Las reglas Deleting se ejecutan siempre antes que las reglas Deleted que están en la cola.  
     ...
     Para obtener más información, vea [Reglas de propagan los cambios en el modelo](../modeling/rules-propagate-changes-within-the-model.md).  
  
2.  El evento de almacén **Deleted** se invoca al final de una transacción y se llama después de Undo o Redo.  Por lo tanto, se puede usar para propagar eliminaciones a objetos fuera del almacén, por ejemplo, archivos, entradas de base de datos u otros objetos de [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)].  
  
     Para obtener más información, vea [Los controladores de eventos propagan cambios fuera del modelo](../modeling/event-handlers-propagate-changes-outside-the-model.md).  
...
### Ejemplo de reglas de eliminación  
