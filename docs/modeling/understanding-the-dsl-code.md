...
> [!NOTE]
>  La plantilla de DSL de diagramas de componentes no está relacionado con los diagramas de componentes UML que puede crear mediante el menú de arquitectura en Visual Studio. En el **nuevo proyecto** diálogo cuadro, expanda **otros tipos de proyecto\extensibilidad** y, a continuación, haga clic en **Diseñador de lenguaje específico de dominio**.  
...
## <a name="the-structure-of-the-dsl-solution"></a>Estructura de la solución de DSL  
 El **Dsl** proyecto define la API para su DSL. El **DslPackage** proyecto define cómo se integra con [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]. También puede agregar sus propios proyectos, que también pueden contener código generado a partir del modelo.  
...
-   Escriba clases parciales en archivos de código diferentes para invalidar los métodos que se definen en clases generadas o se heredan de ellas. En algunos casos, tendrá que configurar la **genera doble derivada** opción de una clase en la definición de DSL para poder invalidar un método generado.  
 ...
 *Sudsl*`Schema.xsd`  
 ...
 Cada clase de generador se genera desde un nodo en el **generadores de conexiones** sección Explorador de DSL. Un método `Connect` puede crear relaciones entre uno o varios pares de clases de dominio. Cada par se define mediante una directiva de conexión de vínculo, que encontrará en DSL Explorer (Explorador de DSL), en el nodo del generador.  
 ...
 Para obtener más información, consulte [generar código desde un lenguaje específico de dominio](../modeling/generating-code-from-a-domain-specific-language.md) y [procesadores de la directiva de plantilla de creación personalizada T4 texto](../modeling/creating-custom-t4-text-template-directive-processors.md).  
  
 `DomainClasses.cs`  
  
 Implementaciones de las clases de dominio que ha definido, incluidas las clases abstractas y la clase raíz del modelo. Se deriva de <xref:Microsoft.VisualStudio.Modeling.ModelElement>.</xref:Microsoft.VisualStudio.Modeling.ModelElement>  
...
-   Constructores. Si desea invalidar estos, establezca **ha personalizado Constructor** en la clase de dominio.  
...
-   Métodos de controlador de Prototipo de grupo de elementos (EGP). Estos son necesarios si el usuario puede *mezcla* (Agregar) otro elemento en instancias de esta clase. Para hacerlo, normalmente el usuario arrastra desde una herramienta de elemento u otra forma, o realiza una operación de pegar.  
     ...
     En el DSL de ejemplo, se puede combinar un puerto de entrada o de salida con un componente. Además, se pueden combinar componentes y comentarios en el modelo. La colección  
 ...
 La clase que representa el modelo de dominio. Se deriva de <xref:Microsoft.VisualStudio.Modeling.DomainModel>.</xref:Microsoft.VisualStudio.Modeling.DomainModel>  
  
> [!NOTE]
>  No es la misma que la clase raíz del modelo.  
  
 Copy y Delete Closures definen qué otros elementos se deben incluir cuando se copia o se elimina un elemento. Puede controlar este comportamiento estableciendo el **Propagates Copy** y **propaga eliminar** propiedades de las funciones en cada lado de cada relación. Si quiere que los valores se determinen dinámicamente, puede escribir código para invalidar los métodos de las clases Closure. 
 ...
 Cada vínculo entre dos elementos de un modelo se representa con una instancia de una clase de relación de dominio. Todas las clases de relación se derivan <xref:Microsoft.VisualStudio.Modeling.ElementLink>, que a su vez se deriva de <xref:Microsoft.VisualStudio.Modeling.ModelElement>.</xref:Microsoft.VisualStudio.Modeling.ModelElement> </xref:Microsoft.VisualStudio.Modeling.ElementLink> Como es un ModelElement, una instancia de una relación puede tener propiedades y puede ser el origen o el destino de una relación.  
 ...
 Para que estas restricciones que se ejecute, debe haber configurado uno de los **usa... ** las opciones de la **Editor\Validation** nodo en el Explorador de DSL. Para obtener más información, consulte [validación en un lenguaje específico de dominio](../modeling/validation-in-a-domain-specific-language.md).  
  
 `PropertiesGrid.cs`  
  
 Este archivo contiene código solo si ha asociado un descriptor de tipo personalizado a una propiedad de dominio. Para obtener más información, consulte [personalización de la ventana propiedades](../modeling/customizing-the-properties-window.md).  
...
-   Clase SerializationHelper, que proporciona funciones que las clases de serialización usan en común.  
 ...
 Una clase para cada clase de forma en la definición de DSL. Las formas derivan <xref:Microsoft.VisualStudio.Modeling.Diagrams.NodeShape>.</xref:Microsoft.VisualStudio.Modeling.Diagrams.NodeShape> Para obtener más información, consulte [personalizar el almacenamiento de archivos y serialización XML](../modeling/customizing-file-storage-and-xml-serialization.md).  
  
 Para reemplazar los métodos generados con sus propios métodos en una clase parcial, establezca **genera doble derivada** para el conector en la definición de DSL. Para reemplazar un constructor por su propio código, establezca **ha personalizado Constructor**.  
  
 Para que el color y la otra variable de características de estilo en tiempo de ejecución, haga clic en la clase en el diagrama de la definición de DSL y seleccione **agregar expone**.  
  
 Por ejemplo para que sean variables de características de estilo adicionales en tiempo de ejecución, consulte <xref:Microsoft.VisualStudio.Modeling.Diagrams.TextField>y <xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement></xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement> </xref:Microsoft.VisualStudio.Modeling.Diagrams.TextField>  
 ...
 *Sudsl* `DocData` administra la carga y guardar un modelo al archivo y crea la instancia del almacén.  
