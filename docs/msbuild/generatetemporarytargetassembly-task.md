...
dev_langs: 
  ...
  - "jsharp"
helpviewer_keywords: 
  - "proceso de compilación [WPF MSBuild], referencia de la página XAML a un tipo declarado localmente"
  - "crear un ensamblado [WPF MSBuild], referencia de la página XAML a un tipo declarado localmente"
  - "GenerateTemporaryTargetAssembly (tarea) [WPF MSBuild]"
  - "GenerateTemporaryTargetAssembly (tarea) [WPF MSBuild], parámetros"
...
|`GeneratedCodeFiles`|Parámetro opcional de tipo **ITaskItem\[\]**.<br /><br /> Especifica la lista de archivos de código fuente administrados específicos del lenguaje y generados por la tarea [MarkupCompilePass1](../msbuild/markupcompilepass1-task.md).|  
