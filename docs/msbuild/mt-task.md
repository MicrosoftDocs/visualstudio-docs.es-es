...
|**AssemblyIdentity**|Parámetro **String** opcional.<br /><br /> Especifica los valores de atributo del elemento **assemblyIdentity** del manifiesto. Especifique una lista delimitada por comas, donde el primer componente sea el valor del atributo `name`, seguido de uno o varios pares de nombre/valor que tengan el formato *\<attribute name>=<valor_atributo>* .<br /><br /> Para más información, consulte la opción **/identity** de [Mt.exe](/windows/desktop/SbsCs/mt-exe).|
