...
description: Obtenga información sobre cómo MSBuild usa destinos para agrupar tareas y permitir que el proceso de compilación se factorice en unidades más pequeñas.
...
MSBuild depende del orden de importación y la última definición de un destino es la definición usada. Si intenta volver a definir un destino, no tendrá efecto si después se define el destino integrado. En el caso de los proyectos que usan un SDK, el orden de definición no es necesariamente obvio, ya que las importaciones de los destinos se agregan de forma implícita después del final del archivo del proyecto.

Por tanto, para ampliar el comportamiento de un destino existente, cree un destino y especifique `BeforeTargets` (o `AfterTargets` según corresponda) como se muestra a continuación:
...
Asigne un nombre descriptivo al destino, como haría con una función en el código.
...
Un elemento de destino puede tener un atributo `Outputs` que especifique los metadatos en el formato %(\<Metadata>). Si es así, MSBuild ejecuta el destino una vez para cada valor de metadatos único, y agrupa o "procesa por lotes" los elementos que tienen ese valor de metadatos. Por ejemplo,
