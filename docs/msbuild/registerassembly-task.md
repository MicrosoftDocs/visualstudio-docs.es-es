...
dev_langs: 
  ...
  - "jsharp"
helpviewer_keywords: 
  - "MSBuild, RegisterAssembly (tarea)"
  - "RegisterAssembly (tarea) [MSBuild]"
...
|`AssemblyListFile`|Parámetro <xref:Microsoft.Build.Framework.ITaskItem> opcional.<br /><br /> Contiene información sobre el estado entre las tareas `RegisterAssembly` y [UnregisterAssembly](../msbuild/unregisterassembly-task.md).  De esta forma, se evita que la tarea `UnregisterAssembly` trate de anular el registro de un ensamblado que no se registró en la tarea `RegisterAssembly`.|  
