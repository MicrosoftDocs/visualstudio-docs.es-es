...
|`Platform`|Parámetro `String` opcional.<br /><br /> Limita en qué plataforma se puede ejecutar este código; debe ser `x86`, `Itanium`, `x64` o `anycpu`. El valor predeterminado es `anycpu`. Este parámetro corresponde a la opción `/platform` de [Al.exe (Assembly Linker)](/dotnet/framework/tools/al-exe-assembly-linker).|  
