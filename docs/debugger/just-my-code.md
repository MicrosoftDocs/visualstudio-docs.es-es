...
##  <a name="BKMK_Enable_or_disable_Just_My_Code"></a> Habilitar o deshabilitar solo mi código  
 Para habilitar o deshabilitar solo mi código, elija la **Herramientas > opciones** menú en Visual Studio. En el **depuración** > **General** nodo, elija o borre **habilitar solo mi código**.
  
 ![Habilitar sólo mi código en el cuadro de diálogo Opciones](../debugger/media/dbg_justmycode_options.png "DBG_JustMyCode_Options")  
  
> [!NOTE]
>  El **habilitar solo mi código** es una configuración global que se aplica a todos los proyectos de Visual Studio en todos los idiomas.  
  
###  <a name="BKMK_Override_call_stack_filtering"></a> Mostrar código de no usuario en vistas de la pila de llamadas  
 En las vistas que muestran la pila de llamadas, como el **pila de llamadas** y **tareas** windows, solo mi código contrae el código no es de usuario que en un marco anotado con la etiqueta `[External Code]`. Para ver los marcos contraídos, elija **mostrar código externo** en el menú contextual de la pila de llamadas para mostrar.
...
###  <a name="BKMK_NET_User_and_non_user_code"></a> Código de usuario y no es de usuario  
 Para distinguir entre el código de usuario desde el código de no usuario, solo mi código examina los archivos de símbolos (.pdb) y las optimizaciones de programa. El depurador considera el código como código que no es de usuario cuando se optimiza el archivo binario o cuando el archivo .pdb no está disponible.
...
###  <a name="BKMK_CPP_User_and_non_user_code"></a> Código de usuario y no es de usuario  
...
###  <a name="BKMK_CPP_Exception_behavior"></a> Comportamiento de excepciones  
 Cuando el depurador llega a una excepción, se detiene en la excepción independientemente de si está en el usuario o el código de no usuario. El **no controlada por el usuario** opciones en el **excepciones** se pasan por alto el cuadro de diálogo.  
...
-   Para especificar el código de no usuario para todos los usuarios del equipo de Visual Studio, agregue el archivo .natstepfilter para el `%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers` carpeta.  
  
-   Para especificar el código de no usuario para un usuario individual, agregue el archivo .natstepfilter a la `%USERPROFILE%\My Documents\Visual Studio 2015\Visualizers` carpeta.  
...
-   Para especificar el código de no usuario para todos los usuarios del equipo de Visual Studio, agregue el archivo .natjmc para el `%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers` carpeta.  
  
-   Para especificar el código de no usuario para un usuario individual, agregue el archivo .natjmc a la `%USERPROFILE%\My Documents\Visual Studio 2015\Visualizers` carpeta.  
...
###  <a name="BKMK_JS_User_and_non_user_code"></a> Código de usuario y no es de usuario  
...
-   Si una función no es de usuario (**MyCode**) código, **paso a paso** (método abreviado de teclado: F11) se comporta como **paso a paso por** (teclado: F10).  
  
-   Si un paso se inicia en no es de usuario (**LibraryCode** o **UnrelatedCode**) de código, ejecución paso a paso temporalmente se comporta como si sólo mi código no está habilitado. Cuando se pasa al código de usuario, solo mi código se vuelve a habilitar la ejecución paso a paso.  
  
-   Cuando un paso en código de usuario hace que se salga del contexto de ejecución actual (como un paso en la última línea de un controlador de eventos), el depurador se detiene en la siguiente línea de código de usuario ejecutada. Por ejemplo, si una devolución de llamada se ejecuta en **LibraryCode** código el depurador continúa hasta que se ejecuta la siguiente línea de código de usuario.
  
-   **Paso a paso fuera** (teclado: MAYÚS + F11) se detiene en la siguiente línea de código de usuario. Si no se encuentra ningún código de usuario, la ejecución continúa hasta que la aplicación se cierra, se visita un punto de interrupción o se produce una excepción.  
  
###  <a name="BKMK_JS_Breakpoint_behavior"></a> Comportamiento de punto de interrupción  
  
-   Puntos de interrupción establecidos en el código siempre se visitan independientemente de la clasificación de ese código  
  
-   Si se encuentra la palabra clave `debugger` en:  
  
    -   **LibraryCode** código, el depurador siempre interrumpe.  
  
    -   **UnrelatedCode** código, el depurador no se detiene.  
...
-   **UnrelatedCode** código, y **MyCode** o **LibraryCode** código está en la pila de llamadas, el depurador se interrumpa.  
  
 Si están habilitadas excepciones de primera oportunidad para la excepción en el cuadro de diálogo de excepciones y la excepción se produce en **LibraryCode** o **UnrelatedCode** código:  
...
3.  Clasificaciones en el archivo `mycode. json` del proyecto actual.  
 ...
 Cada paso de clasificación invalida los pasos anteriores. No necesita un archivo .json enumerar todos los pares clave / valor y el **MyCode**, **bibliotecas**, y **Unrelated** los valores pueden ser matrices vacías.  
 ...
 **Eval, Function y ScriptBlock**  
 ...
 El **Eval**, **función**, y **ScriptBlock** , determine cómo dinámicamente los pares clave / valor se clasifica el código generado.  
