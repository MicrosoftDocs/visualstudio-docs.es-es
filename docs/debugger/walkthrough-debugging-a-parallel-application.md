...
1. En el menú **Depurar**, seleccione **Ventanas** y, a continuación, haga clic en **Pilas paralelas**. Asegúrese de que **Subprocesos** está seleccionado en el cuadro de la esquina superior izquierda.
     ...
     Mediante el uso de la ventana **pilas paralelas** , puede ver varias pilas de llamadas al mismo tiempo en una vista. En la ilustración siguiente se muestra la ventana **pilas paralelas** encima de la ventana **pila de llamadas** .

     ![Vista de subprocesos en la ventana pilas paralelas](../debugger/media/pdb_walkthrough_1.png "PDB_Walkthrough_1")

     La pila de llamadas del subproceso principal aparece en un cuadro y las pilas de llamadas de los otros cuatro subprocesos están agrupadas en otro cuadro. Se agrupan cuatro subprocesos porque sus marcos de pila comparten los mismos contextos de método, es decir, están en los mismos métodos: `A`, `B` y `C`. Para ver los identificadores y nombres de subprocesos de los subprocesos que comparten el mismo cuadro, mantenga el mouse sobre el cuadro con el encabezado (**4 subprocesos**). El subproceso actual se muestra en negrita.

     ![Información sobre herramientas que muestra los nombres y los identificadores de subprocesos](../debugger/media/pdb_walkthrough_1a.png "PDB_Walkthrough_1A")
...
1. Para reanudar la ejecución hasta llegar al segundo punto de interrupción, haga clic en **Continuar**, en el menú **Depurar**. La siguiente ilustración muestra el árbol de subproceso en el segundo punto de interrupción.
     ...
     ![Ventana pilas paralelas que muestra muchas ramas](../debugger/media/pdb_walkthrough_2.png "PDB_Walkthrough_2")
     ...
     El icono de subprocesos de tela (líneas interprecisas) indica los marcos de pila activos de los subprocesos no actuales. En la ventana **Pila de llamadas**, haga doble clic en S.B para intercambiar los marcos. La ventana **Pilas paralelas** indica el marco de pila del subproceso actual utilizando un icono de flecha verde y curvada.
     ...
     ![Ruta de acceso de pilas paralelas de la ejecución](../debugger/media/pdb_walkthrough_2b.png "PDB_Walkthrough_2B")
     ...
     ![Menú pilas con 2 opciones en C mientras J es actual](../debugger/media/pdb_walkthrough_3.png "PDB_Walkthrough_3")
     ...
     ![Ventana de pilas paralelas e información sobre herramientas](../debugger/media/pdb_walkthrough_3a.png "PDB_Walkthrough_3A")

     Ahora, en la ventana **pilas paralelas** solo se muestra el subproceso marcado.
...
1. Para reanudar la ejecución hasta llegar al tercer punto de interrupción, haga clic en **Continuar**, en el menú **Depurar**.
     ...
     Observe que S.L está en negrita en los otros dos cuadros para que pueda ver dónde más aparece. Si quiere ver qué marcos llaman a S.L y a qué marcos llama, haga clic en el botón **Alternar vista de método** en la barra de herramientas. En la ilustración siguiente se muestra la vista de método de la ventana **pilas paralelas** .

     ![Vista de métodos en la ventana pilas paralelas](../debugger/media/pdb_walkthrough_4.png "PDW_Walkthrough_4")
    ...
    - **Mostrar subprocesos en origen** alterna la presentación de los marcadores de subprocesos en el código fuente, que muestra la ubicación de los subprocesos en el código fuente.
...
1. Para reanudar la ejecución hasta llegar al cuarto punto de interrupción, haga clic en **Continuar** en el menú **Depurar**.
     ...
     La **Vista aérea** también permite ver diagramas grandes en la ventana **Pilas paralelas**. De forma predeterminada, la **vista de pájaro** está activada. Pero se puede alternar haciendo clic en el botón situado entre las barras de desplazamiento en la esquina inferior derecha de la ventana, tal como se muestra en la siguiente ilustración.

     ![Vista de&#45;pájaro en la ventana pilas paralelas](../debugger/media/pdb_walkthrough_5.png "PDB_Walkthrough_5")

     En la vista de pájaro, puede mover el rectángulo para desplazarse rápidamente por el diagrama.
...
5. En el menú **depurar** , seleccione **ventanas**y, a continuación, haga clic en **tareas**. En la ilustración siguiente se muestra la ventana **tareas** .
...
1. Para reanudar la ejecución hasta llegar al segundo punto de interrupción, haga clic en **Continuar**, en el menú **Depurar**.
     ...
     ![Vista de tareas en la ventana pilas paralelas](../debugger/media/pdb_walkthrough_8.png "PDB_Walkthrough_8")
     ...
     Vuelva a ver la ventana **tareas** . Haga clic con el botón secundario en cualquier encabezado de columna para ver un menú contextual de la columna.
...
1. Para reanudar la ejecución hasta llegar al tercer punto de interrupción, haga clic en **Continuar**, en el menú **Depurar**.
     ...
     Ahora se está ejecutando una nueva tarea, la tarea 5, y la tarea 4 está en espera. Puede ver por qué desplazando el puntero del mouse sobre la tarea en espera de la ventana **Estado**. En la columna **primaria** , observe que tarea 4 es el elemento primario de la tarea 5.

     Para visualizar mejor la relación de elementos primarios y secundarios, haga clic con el botón secundario en la fila de encabezado de columna y, a continuación, haga clic en **vista primaria y secundaria**. Vea la ilustración siguiente:

     ![Vista&#45;de elementos primarios y secundarios en la ventana tareas](../debugger/media/pdb_walkthrough_9.png "PDB_Walkthrough_9")

     Tenga en cuenta que las tareas 4 y 5 se están ejecutando en el mismo subproceso (muestre la columna **asignación de subprocesos** si está oculta). Esta información no se muestra en la ventana **subprocesos** ; verlo aquí es otra ventaja de la ventana **tareas** . Para confirmar esto, vea la ventana **Pilas paralelas**. Asegúrese de que está viendo **Tareas**. Busque las tareas 4 y 5 haciendo doble clic en ellas en la ventana **tareas** . Al hacerlo, se actualiza el resaltado azul de la ventana **Pilas paralelas**. También puede buscar las tareas 4 y 5 examinando la información sobre herramientas en la ventana **Pilas paralelas**.

     ![Vista de tareas en la ventana pilas paralelas](../debugger/media/pdb_walkthrough_9a.png "PDB_Walkthrough_9A")
...
1. Para reanudar la ejecución hasta llegar al tercer punto de interrupción, haga clic en **Continuar**, en el menú **Depurar**. Haga clic en el encabezado de columna **Id.** para ordenar por identificador. Vea la ilustración siguiente:
     ...
     Puede agrupar las tareas por columna. En la ventana **tareas** , haga clic con el botón secundario en el encabezado de columna **Estado** y, a continuación, haga clic en **Agrupar por estado**. En la ilustración siguiente se muestra la ventana **tareas** agrupadas por estado.

     ![Tareas agrupadas en la ventana tareas](../debugger/media/pdb_walkthrough_12.png "PDB_Walkthrough_12")
     ...
     La última característica de la ventana **tareas** que se va a examinar es el menú contextual que se muestra al hacer clic con el botón secundario en una tarea.
     ...
     Puede inmovilizar el subproceso subyacente de una o varias tareas, y puede inmovilizar todos los subprocesos exceptuando el asignado. Un subproceso inmovilizado se representa en la ventana **tareas** tal como está en la ventana **subprocesos** , mediante un icono de *pausa* azul.
