---
title: Iniciar una sesión de depuración para una aplicación para UWP en Visual Studio | Microsoft Docs
ms.custom: ''
ms.date: 01/04/2018
...
ms.locfileid: "49818789"
---
# <a name="start-a-debugging-session-for-a-uwp-app-in-visual-studio"></a>Iniciar una sesión de depuración para una aplicación para UWP en Visual Studio
  
 Este tema describe cómo iniciar una sesión de depuración para aplicaciones UWP escritas en XAML y Visual C++, Visual C# o Visual Basic y para aplicaciones UWP escritas en HTML y JavaScript. Para depurar una aplicación, hay que configurar la sesión de depuración y elegir la manera de iniciar la aplicación.  
...
|**Simulador**|Depurar la aplicación en el simulador de Visual Studio para aplicaciones UWP. El simulador es una ventana del escritorio que le permite depurar la funcionalidad del dispositivo, como los gestos táctiles y rotación de dispositivos, que no estén disponibles en el equipo local. Esta opción solo está disponible si la aplicación **mínima de la plataforma de destino. Versión** es menor o igual que el sistema operativo en el equipo de desarrollo. Consulte [ejecutar aplicaciones para UWP en el simulador](../debugger/run-windows-store-apps-in-the-simulator.md).|  
|**Equipo remoto**|Depura la aplicación en un dispositivo que esté conectado al equipo local a través de la intranet o directamente mediante un cable Ethernet. Para depurar de forma remota, las herramientas remotas para Visual Studio debe estar instalado y ejecutándose en el dispositivo remoto. Consulte [ejecutar aplicaciones para UWP en un equipo remoto](../debugger/run-windows-store-apps-on-a-remote-machine.md).|  
|**Device**|Depurar la aplicación en un dispositivo USB conectado. El dispositivo debe ser desarrollador y tener la pantalla desbloqueada.|  
|**Emulador de dispositivos móvil**|Inicie un emulador con la configuración especificada en el nombre del emulador, implemente la aplicación e iniciar la depuración. Los emuladores solo están disponibles en equipos de Hyper-V habilitado.|  

##  <a name="BKMK_Open_the_debugging_property_page_for_the_project"></a> Elija otras opciones de depuración  

Si tiene que configurar otras opciones de depuración, abra la página de propiedades del proyecto.
  
1.  En el Explorador de soluciones, seleccione el proyecto. Elige **Propiedades**en el menú contextual.  
  
2.  Haga esto para abrir la página de propiedades Depurar del proyecto:  
    ...
    -   Para las aplicaciones de Visual C++ y JavaScript, expanda el **propiedades de configuración** nodo y, a continuación, elija **depuración**.  
...
|**Script y nativo**|Depurar código de C/C ++ nativo y código de JavaScript de la aplicación. Se omite el código administrado. Está disponible en los proyectos de C++.|  
...
En las aplicaciones de C# y Visual Basic, se puede establecer el mismo **el tipo de depurador** valores para las tareas en segundo plano que forman parte del proyecto.
...
-   Para las aplicaciones de Visual C++ y JavaScript, elija **No** desde el **permitir bucle invertido de red Local** lista el **depuración** página de propiedades.  
...
###  <a name="BKMK__Optional__Disable_authentication_requirement_to_start_the_remote_debugger"></a> (Opcional) Deshabilitar el requisito de autenticación para iniciar el depurador remoto  
 ...
 De forma predeterminada, debe proporcionar las credenciales para ejecutar el depurador remoto cuando se selecciona **máquina remota** como el destino de implementación.
  
> [!IMPORTANT]
>  Puede elegir ejecutar el depurador remoto sin autenticación, pero se recomienda no usar este modo. No hay ninguna seguridad de red cuando se ejecuta en este modo. No elija autenticación solo si está seguro de que la red no está en riesgo de tráfico hostil o código malintencionado.  
  
 Para quitar el requisito de autenticación:  
  
1.  Para las aplicaciones de Visual C# y Visual Basic, seleccione **máquina remota** como el **dispositivo de destino** en el **depurar** página de propiedades y, después, establezca **modo de autenticación**  a **ninguno** o **Universal (protocolo sin cifrar)**.
  
2.  Para las aplicaciones de Visual C++ y JavaScript, seleccione **máquina remota** como el **dispositivo de destino** en el **depuración** página de propiedades y, después, establezca **requieren Autenticación** a **ninguno** o **Universal (protocolo sin cifrar)**.  

    **Universal (protocolo sin cifrar)** es para su uso cuando se va a implementar en un dispositivo remoto. Actualmente, esto es para dispositivos de IoT, Xbox dispositivos y los dispositivos HoloLens, así como Creators Update o equipos más recientes. Universal (protocolo sin cifrar) solo debe usarse en redes de confianza. La conexión de depuración es vulnerable a los usuarios malintencionados que podrían interceptar y cambiar los datos que se pasan entre el desarrollo y el equipo remoto.  
...
###  <a name="BKMK_Start_debugging__F5__but_delay_the_app_start"></a> Iniciar la depuración (F5) pero retrasar el inicio de la aplicación  
 Puedes establecer que la aplicación se ejecute en modo de depuración, pero iniciarla mediante un método que no sea el depurador. Por ejemplo, es posible que desee para depurar el inicio de la aplicación desde el menú Inicio, o para depurar un proceso en segundo plano en la aplicación sin iniciar la aplicación. Para retrasar el inicio de la aplicación, puede hacer esto:  
...
- En el **depurar** página de propiedades de la aplicación (**depuración** en Visual C++ y JavaScript)  
  ...
  -   Para las aplicaciones de Visual C++ y JavaScript, elija **Sí** desde el **Iniciar aplicación** lista.  
...
###  <a name="BKMK_Attach_the_debugger_to_a_running_app_"></a> Asociar al depurador a una aplicación para UWP en ejecución  

Para depurar una aplicación para UWP en ejecución, elija **depurar**, a continuación, **otros destinos de depuración**y, a continuación, **depurar paquete de aplicaciones instalado**. Para obtener instrucciones, consulte [depurar un paquete de aplicación instalados](../debugger/debug-installed-app-package.md).
...
1.  Instalar las herramientas remotas para Visual Studio en el dispositivo donde está instalada la aplicación. Consulte [instalar las herramientas remotas](../debugger/remote-debugging.md).  
...
4. En la lista **Procesos disponibles**  , elige el proceso de aplicación.  
   ...
   > [!NOTE]
   >  A diferencia de otros tipos de aplicaciones, las aplicaciones de JavaScript se ejecutan en una instancia del proceso wwahost.exe. Si hay otras aplicaciones de JavaScript ejecutándose durante la asociación, debe conocer el identificador numérico del proceso (PID) del proceso wwahost.exe en el que se ejecuta la aplicación.  
   >   
