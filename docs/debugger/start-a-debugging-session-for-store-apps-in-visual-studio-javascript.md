---
title: "Iniciar una sesión de depuración para aplicaciones de UWP en Visual Studio (JavaScript) | Documentos de Microsoft"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
---
# <a name="start-a-debugging-session-for-uwp-apps-in-visual-studio-javascript"></a>Iniciar una sesión de depuración para aplicaciones de UWP en Visual Studio (JavaScript)
![Se aplica a Windows y Windows Phone](../debugger/media/windows_and_phone_content.png "windows_and_phone_content")  
  
 En este tema se describe cómo iniciar una sesión de depuración para aplicaciones UWP escritas en JavaScript y HTML5. Puede iniciar la depuración presionando una sola tecla o configurar la sesión de depuración para situaciones concretas y, a continuación, elegir el modo de iniciar la aplicación.  
  
> [!NOTE]
>  Para aplicaciones escritas en XAML y Visual C#, Visual C++ o Visual Basic, consulte [inicie una sesión de depuración (VB, C#, C++ y XAML)](../debugger/start-a-debugging-session-for-a-store-app-in-visual-studio-vb-csharp-cpp-and-xaml.md)  
...
2.  Si la solución contiene varios proyectos, asegúrese de que el proyecto que va a depurar sea el proyecto de inicio. En el Explorador de soluciones, seleccione el proyecto y, a continuación, elija **establecer como proyecto de inicio** en el menú contextual.  
  
3.  Presione F5.  
  
 ![Solo se aplica a Windows Phone](../debugger/media/phone_only_content.png "phone_only_content")  
...
##  <a name="BKMK_Configure_the_debugging_session"></a> Configurar la sesión de depuración  
 Dado que no se compila el script, no se aplicará la configuración de la compilación y de la plataforma. Si está depurando un componente administrado o C++, establezca el **configuración** a **depurar** y elija la plataforma de destino de la **configuración** cuadro de diálogo.  
...
###  <a name="BKMK_Choose_the_deployment_target"></a> Elegir el destino de implementación  
 Puede implementar y depurar una aplicación en el equipo de Visual Studio, en el simulador de Visual Studio del equipo local o en un equipo remoto. Seleccione el destino de la **depurador para iniciar** lista el **depuración** página de propiedades para el proyecto.  
  
 ![Solo se aplica a Windows](../debugger/media/windows_only_content.png "windows_only_content")  
  
 Para una aplicación UWP, elija una de estas opciones de la **dispositivo de destino** lista:  
  
|||  
|-|-|  
|**Equipo local**|Depura la aplicación en la sesión actual en el equipo local. Vea [UWP ejecutar aplicaciones en el equipo local](../debugger/run-windows-store-apps-on-the-local-machine.md).|  
|**Simulador**|Depura la aplicación en el simulador de Visual Studio para aplicaciones de la [!INCLUDE[win8_appname_long](../debugger/includes/win8_appname_long_md.md)] . El simulador es una ventana del escritorio que te permite depurar la funcionalidad del dispositivo (por ejemplo, gestos táctiles y de rotación de dispositivos) que no está disponible en el equipo local. Vea [UWP ejecutar aplicaciones en el simulador](../debugger/run-windows-store-apps-in-the-simulator.md).|  
...
-   Elija la flecha hacia abajo en la **nombre de la máquina** cuadro y elija  **\<buscar... >**. A continuación, elija el equipo remoto en **Seleccionar conexión del depurador remoto** cuadro de diálogo.  
  
     ![Seleccionar conexión del depurador remoto](../debugger/media/vsrun_pro_selectremotedebuggerdlg.png "VSRUN_PRO_SelectRemoteDebuggerDlg")  
 ...
 Para las aplicaciones de Windows Phone, elija **dispositivo** o uno de los emuladores de la **dispositivo de destino** lista.  
...
###  <a name="BKMK__Optional__Delay_starting_app_in_the_debug_session"></a>(Opcional) Retrasar el inicio de la aplicación en la sesión de depuración  
 ...
 Especifique si desea retrasar el inicio de la aplicación en el **Iniciar aplicación** lista el **depuración** página de propiedades de proyecto de la aplicación. Elige una de estas opciones:  
  
-   Elija **n** para retrasar el inicio de la aplicación.  
...
###  <a name="BKMK_Start_debugging__F5_"></a> Iniciar la depuración (F5)  
 Cuando eliges **Iniciar depuración** en el **depurar** menú (teclado: F5), Visual Studio inicia la aplicación con el depurador adjunto. La ejecución continúa hasta que se alcanza un punto de interrupción, se suspende manualmente la ejecución, se produce una excepción no controlada o la aplicación finaliza.  
...
1.  En el **depurar** página de la aplicación de propiedades del proyecto, elija **No** desde el **Iniciar aplicación** lista.  
...
1.  En el **depurar** menú, elija **depurar paquete de aplicaciones instalado**.  
...
1.  Instala las Herramientas remotas de Visual Studio en el dispositivo donde esté instalada la aplicación. Vea [instalar las herramientas remotas](http://msdn.microsoft.com/library/windows/apps/hh441469.aspx#BKMK_Installing_the_Remote_Tools).  
...
2.  Para asociarlo a una aplicación de un dispositivo remoto, especifícalo en el cuadro **Calificador** . Puede realizar lo siguiente:  
    ...
    -   Elija la flecha hacia abajo en la **calificador** cuadro y elija el dispositivo en una lista de dispositivos asociados a antes.  
...
3.  Especifica el tipo de código que deseas depurar en el cuadro **Asociar a** .  
    ...
    -   Elija **determinar automáticamente el tipo de código para depurar**.  
