...
Las herramientas que necesita usar en [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] dependen del tipo de código que está intentando depurar:
...
- En el caso de los subprocesos, las herramientas principales para depurar subprocesos son la ventana **subprocesos** , los marcadores de subprocesos de las ventanas de código fuente, la ventana **pilas paralelas** , la ventana **inspección paralela** y la barra de herramientas **Ubicación** de

- En el caso del código que usa el <xref:System.Threading.Tasks.Task> en la [biblioteca TPL](/dotnet/standard/parallel-programming/task-parallel-library-tpl), el [Runtime de simultaneidad](/cpp/parallel/concrt/concurrency-runtime/) (código nativo), las herramientas principales para depurar aplicaciones multiproceso son la ventana **pilas paralelas** , la **inspección paralela** y la ventana **tareas** (la ventana **tareas** también admite el objeto Promise de JavaScript).

- Para depurar subprocesos en la GPU, la herramienta principal es las ventanas de **subprocesos de GPU** .
...
|Ventana **Pilas paralelas**|-   Pilas de llamadas para varios subprocesos en una ventana.<br />-   Marco de pila activa para cada subproceso.<br />-   Llamadores y destinatarios de cualquier método|-   Filtrar subprocesos especificados<br />-Cambiar a la vista de tareas<br />-   Marcar o demarcar un subproceso<br />-   Zoom|
