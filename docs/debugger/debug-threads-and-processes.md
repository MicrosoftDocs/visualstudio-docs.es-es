...
Las herramientas que necesita usar en [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] dependen del tipo de código que esté intentando depurar:
...
- En el caso de los subprocesos, las herramientas principales para depurar subprocesos son la ventana **Subprocesos**, los marcadores de subprocesos en ventanas de código fuente, la ventana **Pilas paralelas**, la ventana **Inspección paralela** y la barra de herramientas **Ubicación de depuración**.

- En el caso del código que usa la <xref:System.Threading.Tasks.Task> de la [Biblioteca de procesamiento paralelo basado en tareas (TPL)](/dotnet/standard/parallel-programming/task-parallel-library-tpl), el [Runtime de simultaneidad](/cpp/parallel/concrt/concurrency-runtime/) (código nativo), las herramientas principales para depurar aplicaciones multiproceso son la ventana **Pilas paralelas**, la ventana **Inspección paralela** y la ventana **Tareas** (la ventana **Tareas** también admite el objeto Promise de JavaScript).

- Para depurar subprocesos en la GPU, la herramienta principal es la ventana **Subprocesos de GPU**.
