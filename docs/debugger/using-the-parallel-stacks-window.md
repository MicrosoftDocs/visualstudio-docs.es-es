---
title: Ver conversaciones mediante la ventana Pilas paralelas | Microsoft Docs
ms.custom: ''
ms.date: 04/25/2017
...
## <a name="threads-view"></a>Vista de subprocesos  
 ...
 En la ilustración, la ruta de acceso de llamada del subproceso actual se resalta en azul y la ubicación actual (marco de pila activo) del subproceso está indicada por la flecha amarilla. Puede cambiar el marco de pila seleccionando un método diferente en el **pilas paralelas** ventana. Esto también puede producir el cambio del subproceso actual, dependiendo de si el método que seleccionó ya forma parte del subproceso actual o de otro. La tabla siguiente describen las características principales de la **pilas paralelas** ventana, tal como se muestra en la ilustración.  
...
|F|Información sobre herramientas en el método|En la vista de subprocesos, muestra todos los subprocesos en una tabla similar a la **subprocesos** ventana. En la vista de tareas, muestra todas las tareas en una tabla similar a la **tareas** ventana.|  
...
|E|Alternar control Zoom|Muestra u oculta el control Zoom. También puede hacer zoom presionando CTRL y girando la rueda del mouse, independientemente de la visibilidad del control de zoom, o mediante CTRL + MAYÚS + "+" para acercar y CTRL + MAYÚS +'-' para alejar. Presione CTRL + F8 irá para ajustarse a la pantalla.|  
  
### <a name="context-menu-items"></a>Elementos del menú contextual  
 La siguiente ilustración y tabla describen los elementos de menú contextual que están disponibles cuando hace clic en un método en la vista de subprocesos o la vista de tareas. Los últimos seis elementos se toman prestados directamente en la ventana Pila de llamadas y no presentan nuevo comportamiento.  
