...
helpviewer_keywords: 
  ...
  - "Visual Studio, jerarquía de llamadas (ventana)"
  - "Visual Studio, vista de clases"
  - "Visual Studio, definición de código (ventana)"
  - "Visual Studio, esquema del documento (ventana)"
  - "Visual Studio, examinador de objetos"
...
Puede examinar los objetos y miembros de proyectos de Visual Studio y los objetos y miembros de componentes de .NET Framework, componentes de COM, bibliotecas de vínculos dinámicos \(DLL\) y bibliotecas de tipo \(TLB\).  
 ...
 Si tiene Visual Studio Enterprise puede utilizar mapas de código para visualizar la estructura del código y sus dependencias en toda la solución y explorar en profundidad hasta llegar a las partes del código que le interesen.  Para obtener más información, consulte [Asignar dependencias de sus soluciones](../modeling/map-dependencies-across-your-solutions.md).  
...
##  <a name="BKMK_CallHierarchy"></a> Jerarquía de llamadas \(Visual Basic, C\#, C\+\+\)  
 ...
 Para mostrar la ventana **Jerarquía de llamadas**, puede seleccionar un método \(o propiedad o constructor\) y, a continuación, elegir **Ver jerarquía de clases** en el menú contextual.  La visualización debería tener un aspecto similar al de la siguiente imagen.  
...
Ventana Jerarquía de llamadas  
 ...
 La ventana **Jerarquía de llamadas** no encuentra referencias a grupos de métodos, que incluyen los lugares donde un método se agrega como controlador de eventos o se asigna a un delegado.  Para buscar estas referencias, utilice el comando **Buscar todas las referencias**.  
...
##  <a name="BKMK_ObjectBrowser"></a> Examinador de objetos  
 ...
 Puede filtrar lo que desea ver en el **Examinador de objetos**.  Mediante la lista desplegable situada en la parte superior de la ventana, puede elegir una de las siguientes opciones:  
...
-   Un conjunto de componentes personalizado  
 ...
 El **Examinador de objetos** realiza un seguimiento de las selecciones que haya realizado y puede navegar entre ellas utilizando los botones **Adelante** y **Atrás** de la barra de herramientas.  
  
 Puede utilizar el **Examinador de objetos** para agregar una referencia de ensamblado a una solución abierta seleccionando un elemento \(ensamblado, espacio de nombres, tipo o miembro\) y seleccionando el botón **Agregar referencia** de la barra de herramientas.  
...
|**Copiar**|Copia el nombre completo del elemento.|  
|**Quitar**|Si el ámbito es un conjunto de componentes personalizados, quita el componente seleccionado del ámbito.|  
|**Ordenar alfabéticamente**|Enumera los tipos y miembros según su nombre por orden alfabético.|  
...
##  <a name="BKMK_CodeDefinition"></a> Ventana Definición de código \(C\#, C\+\+\)  
 ...
 Aunque esta ventana es de solo lectura, puede establecer puntos de interrupción o marcadores en ella.  Para modificar la definición mostrada, elija **Editar definición** en el menú contextual.  Esto abre el archivo de origen en el editor de código y mueve el punto de inserción a la línea en la que comienza la definición.  
