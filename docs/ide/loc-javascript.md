...
ms.topic: "article"
helpviewer_keywords: 
  - "etiqueta XML de JavaScript <loc>"
  - "loc (etiqueta XML) [JavaScript]"
...
## Comentarios  
 El elemento `<loc>` debe aparecer al principio del archivo, en la misma sección que el elemento `<reference>`.  Las reglas de uso del elemento `<loc>` son iguales que las del elemento `<reference>`.  Para obtener más información, consulte la sección "Directivas Reference" en [IntelliSense para JavaScript](../ide/javascript-intellisense.md).  
