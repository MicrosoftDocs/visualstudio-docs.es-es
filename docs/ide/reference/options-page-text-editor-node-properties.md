...
ms.topic: "article"
helpviewer_keywords: 
  - "Configuración de opciones de herramientas, propiedades del nodo Editor de texto"
...
En este documento se describen algunas páginas \(o colecciones de propiedades\) asociadas a la categoría **TextEditor**, `DTE.Properties("TextEditor", <Property Page>)`, del cuadro de diálogo **Opciones**.  El título de cada subsección es la llamada que se usa para obtener acceso a la colección `Properties` y, en la tabla de cada subsección, se muestran las propiedades que se encuentran en la colección.  
  
 Las macros de Visual Basic de [Controlar la configuración de opciones](../Topic/Controlling%20Options%20Settings.md) ilustran cómo se muestran las opciones actuales y los valores de cada página del cuadro de diálogo **Opciones**.  
...
|UndoCaretActions|Get\/Set \(Boolean\)|Si es `True`, las operaciones de la fase de reversión incluyen el movimiento del punto de inserción, los comandos de selección, etc., además de las acciones de edición que modifican el búfer.|  
...
|WordWrap|Get\/Set \(Boolean\)|Determina si se ajustan las líneas largas en los límites de las palabras.  Si se establece en `true`, el elemento de propiedades `VirtualSpace` \(de esta lista\) se establecerá en `false`.|  
