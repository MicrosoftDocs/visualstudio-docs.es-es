...
2.  Agregue el método `AssignIconsToSquares()` tal como se indica en el paso anterior.  Puede colocarlo justo debajo del código que agregó en el [Paso 2: Agregar un objeto aleatorio y una lista de iconos](../ide/step-2-add-a-random-object-and-a-list-of-icons.md).  
    ...
    >  Se usan los nombres "iconLabel" y"control" porque son descriptivos.  Se pueden reemplazar por cualquier nombre sin que ello repercuta en el funcionamiento del código, siempre y cuando se cambie el nombre en cada instrucción del bucle.  
     ...
     Recuerde que, si no está seguro de qué hace una parte del código, puede situar el puntero del mouse sobre un elemento y consultar la información sobre herramientas que aparece.  También puede avanzar por cada línea de código mientras se ejecuta el programa con el depurador de Visual Studio.  Consulte [Cómo depurar paso a paso en Visual Studio](http://msdn.microsoft.com/vstudio/ee672313.aspx) o [Desplazarse por el código con el depurador](../debugger/navigating-through-code-with-the-debugger.md) para obtener más información.  
  
3.  Para rellenar el tablero de juego de iconos, debe llamar al método `AssignIconsToSquares()` en cuanto el programa se inicie.  Si usa Visual C\#, agregue una instrucción bajo la llamada al método `InitializeComponent()` en el *constructor* de `Form1`, de modo que el formulario llame al nuevo método para que se establezca antes de mostrarse.  A los constructores se les llama cuando se crea un nuevo objeto, como una clase o struct.  Consulte [Constructores \(Guía de programación de C\#\)](http://msdn.microsoft.com/library/ace5hbzh.aspx) o [Usar constructores y destructores](http://msdn.microsoft.com/library/2z08e49e%28v=vs.90%29.aspx) en Visual Basic para obtener más información.  
...
5.  Cierre el programa y, a continuación, ejecútelo de nuevo.  Observe que hay iconos diferentes asignados a cada etiqueta, como se muestra en la siguiente ilustración.  
     ...
     ![Juego de formar parejas con iconos aleatorios](../ide/media/express_tut4step3.png "Express\_Tut4Step3")  
