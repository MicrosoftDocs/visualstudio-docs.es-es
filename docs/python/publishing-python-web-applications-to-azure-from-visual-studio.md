...
ms.locfileid: "39468675"
---
# <a name="publish-to-azure-app-service"></a>Publicación en Azure App Service

> [!Important]
> La implementación de aplicaciones de Python en Azure App Service para Linux de momento no se admite desde Visual Studio. Microsoft además tiene previsto dejar en desuso Python en App Service en Windows. Las actualizaciones se publicarán en este artículo cuando estén disponibles. Mientras tanto, puede implementar en App Service en Linux mediante contenedores. Para obtener más información, vea [Implementación de una aplicación web de Python en Web App for Containers](/azure/app-service/containers/quickstart-python).
...
### <a name="use-a-subscription"></a>Uso de una suscripción
...
### <a name="use-a-temporary-app-service"></a>Uso de una instancia de App Service temporal
...
1. En la ficha **Publicar** que aparece, seleccione el destino de publicación:
    ...
    1. Su propia suscripción de Azure: seleccione **Microsoft Azure App Service**, **Seleccionar existente** y **Publicar**. Aparecerá un cuadro de diálogo en el que podrá seleccionar la suscripción y el App Service adecuados. Si la instancia de App Service no aparece, use el perfil de publicación descargado tal como se describe más abajo para usar una instancia de App Service temporal.
    ...
    2. Si usa un App Service temporal en try.azurewebsites.net o necesita usar un perfil de publicación, seleccione el control **\>** para buscar **Importar perfil**, seleccione esa opción y, luego, seleccione **Publicar**. Esto solicita la ubicación del archivo *.publishsettings* descargado anteriormente.
...
1. Cuando se abra el explorador, es posible que vea el mensaje **No se puede mostrar la página. Error interno en el servidor**. Este mensaje indica que el entorno de Python en el servidor no está completamente configurado, en cuyo caso siga estos pasos:
    ...
    1. Consulte de nuevo [Configuración de un entorno de Python en Azure App Service](managing-python-on-azure-app-service.md), asegurándose de que tiene una extensión de sitio de Python adecuada instalada.
    ...
    4. Para obtener una salida de error detallada, agregue la siguiente línea a *web.config* dentro del nodo `<system.webServer>`, que proporciona una salida de error más detallada:

    ```xml
    <httpErrors errorMode="Detailed"></httpErrors>
    ```

    5. Pruebe a reiniciar el App Service después de instalar nuevos paquetes. No es necesario reiniciar al cambiar *web.config*, ya que la instancia de App Service realiza un reinicio automático cada vez que cambia *web.config*.
...
## <a name="publish-to-app-service---visual-studio-2015"></a>Publicar en App Service: Visual Studio 2015
