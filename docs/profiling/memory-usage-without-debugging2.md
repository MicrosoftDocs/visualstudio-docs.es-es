...
La herramienta Uso de memoria se puede ejecutar [con o sin el depurador](../profiling/running-profiling-tools-with-or-without-the-debugger.md). En este artículo, se muestra cómo usar la herramienta Uso de memoria sin el depurador en el **Generador de perfiles de rendimiento** de Visual Studio, que es lo que se recomienda para compilaciones de versión.
