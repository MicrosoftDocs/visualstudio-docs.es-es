---
title: Uso de claves de producto
Author: evanwindom
Ms.author: jaunger
Manager: evelynp
Ms.date: 10/3/2017
...
## <a name="product-keys-overview"></a>Introducción a las claves de producto
...
Si reclamó la cantidad máxima de claves disponible con su suscripción dentro de un período de 24 horas, verá un mensaje de error en el que se le indica que alcanzó su límite. El límite se restablece cada 24 horas y comienza a las 12:01 a.m. hora del Pacífico.

Nuestros estudios revelan que el número de claves de producto que incluye la suscripción es suficiente para la mayoría de suscriptores. Si necesita más claves, puede enviar una solicitud al [servicio al cliente de suscripciones de Visual Studio](https://www.visualstudio.com/subscriptions/support/) que se evaluará caso a caso.

## <a name="locating-and-claiming-product-keys"></a>Búsqueda y reclamación de claves de producto
Debe iniciar sesión en su suscripción de Visual Studio para ver las claves de producto. Encontrará las claves de producto individuales si selecciona el botón azul Obtener clave de un determinado producto en la página [Descargas](https://my.visualstudio.com/downloads).  Todas las claves también se encuentran disponibles en la página [Claves de producto](https://my.visualstudio.com/productkeys). Si hay varias claves para un solo producto, en la columna Notas de la descarga aparecerán unas notas para ayudarle a identificar qué clave se debe usar.

![Obtener clave de la página de descargas](_img/product-keys/download-get-key-cropped.png) A veces se unen varias ediciones del producto en una sola descarga. En estos casos, la clave del producto que se introduce determina la edición del producto que se instalará.
Algunas claves se proporcionan de manera automática, como las claves "estáticas", que puede usar todas las veces necesarias, ya que la activación no es obligatoria. Otras claves se deben reclamar seleccionando el botón Obtener clave del producto en cuestión.
...
2.  Haga clic en la pestaña [Claves de producto](https://my.visualstudio.com/productkeys).
...
![Buscar la clave de producto](_img\product-keys\visio-product-key-cropped.png) En este ejemplo, vamos a usar la barra de búsqueda para encontrar una clave de producto de Visio 2010.
...
**IMPORTANTE: Si la suscripción expira, ya no podrá reclamar nuevas claves ni exportar las claves que ya haya reclamado.** 
