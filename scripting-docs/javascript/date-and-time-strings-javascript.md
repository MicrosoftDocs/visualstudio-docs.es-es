...
## Formato de fechas mediante Intl.DateTimeFormat  
 Internet Explorer 11 incluye compatibilidad para el objeto [Intl.DateTimeFormat \(Objeto\)](../javascript/reference/intl-datetimeformat-object-javascript.md), que forma parte de la [Especificación de API de internacionalización de ECMAScript](http://www.ecma-international.org/ecma-402/1.0/).  Para dar formato a las fechas, puede utilizar este objeto directamente o puede usar la implementación actualizada de [toLocaleDateString \(Método, Date\)](../javascript/reference/tolocaledatestring-method-date-javascript.md) y [toLocaleTimeString \(Método, Date\)](../javascript/reference/tolocaletimestring-method-date-javascript.md).  Estos métodos de [Date \(Objeto\)](../javascript/reference/date-object-javascript.md) utilizan `Intl.DateTimeFormat` internamente para admitir nuevos parámetros opcionales de la configuración regional y otras opciones de formato.  
...
```  
 ...
 Para obtener una lista completa de opciones de formato, vea [Intl.DateTimeFormat \(Objeto\)](../javascript/reference/intl-datetimeformat-object-javascript.md).  
  
## Aplicar formato a fechas  
 Antes de Internet Explorer 11, JavaScript no tenía métodos específicos para dar formato a las fechas y horas.  Para proporcionar su propio formato de fecha para las versiones anteriores del explorador, utilice los métodos [getDay \(Método, Date\)](../javascript/reference/getday-method-date-javascript.md), [getDate \(Método, Date\)](../javascript/reference/getdate-method-date-javascript.md), [getMonth \(Método, Date\)](../javascript/reference/getmonth-method-date-javascript.md) y [getFullYear \(Método, Date\)](../javascript/reference/getfullyear-method-date-javascript.md).  \([getYear \(Método, Date\)](../javascript/reference/getyear-method-date-javascript.md) está obsoleto y no debe usarse\).  
...
```  
 ...
 Puede proporcionar su propio formato de hora mediante los métodos [getHours \(Método, Date\)](../javascript/reference/gethours-method-date-javascript.md), [getMinutes \(Método, Date\)](../javascript/reference/getminutes-method-date-javascript.md), [getSeconds \(Método, Date\)](../javascript/reference/getseconds-method-date-javascript.md) y [getMilliseconds \(Método, Date\)](../javascript/reference/getmilliseconds-method-date-javascript.md).  
...
-   Primero, intenta analizar una cadena de fecha mediante [Formato de fecha ISO](#ISO).  
  
    > [!NOTE]
    >  JavaScript usa una versión simplificada del formato extendido ISO 8601.  
  
-   Si la cadena de fecha no tiene formato ISO, JavaScript intenta analizar la fecha mediante otras [Otros formatos de fecha](#OtherDateFormats).  
...
|`Z`|El valor de esta posición puede ser uno de los siguientes.  Si se omite el valor, se usa la hora UTC.<br /><br /> -   `Z` indica la hora UTC.<br />-   `+hh:mm` indica que la hora de entrada es el desplazamiento especificado después de la hora UTC.<br />-   `-hh:mm` indica que la hora de entrada es el valor absoluto del desplazamiento especificado antes de la hora UTC.||  
 ...
 Para devolver una fecha en formato ISO, puede usar [toISOString \(Método, Date\)](../javascript/reference/toisostring-method-date-javascript.md).  
...
### Años extendidos  
 ...
 [toISOString \(Método, Date\)](../javascript/reference/toisostring-method-date-javascript.md) siempre usa el formato de año extendido para los años anteriores a 0 y posteriores a 9999.  
...
-   Las horas, los minutos y los segundos se separan con dos puntos.  Sin embargo, se pueden omitir algunas de las partes.  Los siguientes valores son válidos: "10:", "10:11" y "10:11:12".  
