...
dev_langs: 
  ...
  - "DHTML"
helpviewer_keywords: 
  - "valores booleanos, flujo del programa"
  - "continue (instrucción)"
  - "break (instrucción)"
  - "flujo de control, acerca del flujo de control"
...
---
# Controlar el flujo del programa (JavaScript)
...
```  
 ...
 Por ejemplo, si tiene una expresión AND como \(\(x \=\= 123\) && \(y \=\= 6\)\), [!INCLUDE[javascript](../javascript/includes/javascript-md.md)] compruebe primero si x es 123.  Si no lo es, toda la expresión no puede ser true, aunque sea igual a 6.  Por tanto, la prueba nunca se realiza y [!INCLUDE[javascript](../javascript/includes/javascript-md.md)] devuelve el valor **false**.  
...
```  
 ...
 Aunque los bucles `for...in` parecen similares a los bucles `For Each...Next` de VBScript, no funcionan de la misma manera.  El bucle **for...in** de [!INCLUDE[javascript](../javascript/includes/javascript-md.md)] recorre en iteración las propiedades de objetos de [!INCLUDE[javascript](../javascript/includes/javascript-md.md)].  El bucle **For Each...Next** de VBScript recorre en iteración los elementos de una colección.  Para recorrer colecciones en [!INCLUDE[javascript](../javascript/includes/javascript-md.md)], debe utilizar el objeto [Enumerator \(Objeto\)](../javascript/reference/enumerator-object-javascript.md) o, si está presente, el método `forEach` del objeto de colección.  Aunque algunos objetos, como los de Internet Explorer, admiten tanto el bucle **For Each...Next**  de VBScript como el bucle **for...in** de [!INCLUDE[javascript](../javascript/includes/javascript-md.md)], la mayoría de los objetos no los admiten.  
