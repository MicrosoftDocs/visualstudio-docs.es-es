---
title: "Math.ATANH (función) (JavaScript) | Documentos de Microsoft"
ms.custom: 
ms.date: 01/18/2017
ms.prod: windows-client-threshold
ms.reviewer: 
ms.suite: 
ms.technology: devlang-javascript
ms.tgt_pltfrm: 
ms.topic: language-reference
dev_langs:
- JavaScript
- TypeScript
- DHTML
ms.assetid: effa2a59-9f2a-4586-8a02-bc8face582c7
caps.latest.revision: "3"
author: mikejo5000
ms.author: mikejo
manager: ghogen
ms.openlocfilehash: 3b1afab88630168573771fcae4710c623ce8d9ba
ms.sourcegitcommit: aadb9588877418b8b55a5612c1d3842d4520ca4c
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 10/27/2017
---
# <a name="mathatanh-function-javascript"></a>Math.atanh (función de JavaScript)
Devuelve la tangente hiperbólica inversa de un número.  
  
## <a name="syntax"></a>Sintaxis  
  
```  
  
Math.atanh(x)   
```  
  
## <a name="remarks"></a>Comentarios  
 Si `x` es `NaN`, el resultado es `NaN`.  
  
 **Se aplica a**: [Math (objeto)](../../javascript/reference/math-object-javascript.md)  
  
## <a name="requirements"></a>Requisitos  
 [!INCLUDE[jsv12](../../javascript/reference/includes/jsv12-md.md)]  
  
## <a name="see-also"></a>Vea también  
 [Función Math.ACOS](../../javascript/reference/math-acos-function-javascript.md)   
 [Función Math.ACOSH](../../javascript/reference/math-acosh-function-javascript.md)   
 [Función Math.ASIN](../../javascript/reference/math-asin-function-javascript.md)   
 [Math.Asinh (función)](../../javascript/reference/math-asinh-function-javascript.md)   
 [Función Math.atan](../../javascript/reference/math-atan-function-javascript.md)   
 [Función Math.ATAN2](../../javascript/reference/math-atan2-function-javascript.md)   
 [Función Math.Cos](../../javascript/reference/math-cos-function-javascript.md)   
 [Math.Cosh (función)](../../javascript/reference/math-cosh-function-javascript.md)   
 [Función Math.sin](../../javascript/reference/math-sin-function-javascript.md)   
 [Math.Sinh (función)](../../javascript/reference/math-sinh-function-javascript.md)   
 [Math.tanh (Función)](../../javascript/reference/math-tanh-function-javascript.md)