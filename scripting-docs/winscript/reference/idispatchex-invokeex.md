...
ms.locfileid: "88144420"
---
# <a name="idispatchexinvokeex"></a>IDispatchEx::InvokeEx
Proporciona acceso a las propiedades y los métodos expuestos por un `IDispatchEx` objeto.  
...
#### <a name="parameters"></a>Parámetros  
 ...
 `lcid`  
 Contexto de la configuración regional en que se interpretan los argumentos. `lcid`Se pasa a `InvokeEx` para permitir que el objeto interprete sus argumentos específicos de una configuración regional.  
 ...
 Marcas que describen el contexto de la `InvokeEx` llamada:  
...
|DISPATCH_CONSTRUCT|El miembro se utiliza como un constructor. (Este es un nuevo valor definido por `IDispatchEx` ). Los valores válidos para `wFlags` son:<br /><br /> DISPATCH_PROPERTYGET DISPATCH_METHOD DISPATCH_PROPERTYPUT DISPATCH_PROPERTYPUTREF DISPATCH_CONSTRUCT|  
 ...
 `pei`  
 Puntero a una estructura que contiene información de excepciones. Esta estructura se debe rellenar si `DISP_E_EXCEPTION` se devuelve. Puede ser null. Consulte la `IDispatch` documentación para obtener una descripción completa de la `EXCEPINFO` estructura.  
 ...
 `IDispatchEx::InvokeEx`proporciona todas las mismas características que `IDispatch::Invoke` y agrega algunas extensiones:  
  
|Value|Significado|
|-|-|  
|DISPATCH_CONSTRUCT|Indica que el elemento se utiliza como un constructor.|  
|`pspCaller`|`pspCaller`Permite al objeto acceder a los servicios proporcionados por el llamador. Los servicios específicos pueden ser controlados por el propio llamador o delegados a los llamadores más arriba en la cadena de llamadas. Por ejemplo, si un motor de scripts dentro de un explorador realiza una `InvokeEx` llamada a un objeto externo, el objeto puede seguir la `pspCaller` cadena para obtener los servicios del motor de script o explorador. (Tenga en cuenta que la cadena de llamadas no es la misma que la cadena de creación, también conocida como cadena de contenedores o cadena de sitios. La cadena de creación puede estar disponible a través de otro mecanismo, como `IObjectWithSite` ).|  
|Puntero `this`|Cuando se establece DISPATCH_METHOD en `wFlags` , puede haber un "parámetro con nombre" para el valor "this". El DISPID se DISPID_THIS y debe ser el primer parámetro con nombre.|  
 ...
 Se `riid` ha quitado el parámetro no utilizado en `IDispatch::Invoke` .  
  
 Se `puArgArr` `IDispatch::Invoke` ha quitado el parámetro de.  
  
 Consulte la `IDispatch::Invoke` documentación de los ejemplos siguientes:  
...
|DISP_E_EXCEPTION|La aplicación debe generar una excepción. En este caso, `pei` se debe rellenar la estructura pasada.|  
...
|DISP_E_NONAMEDARGS|Esta implementación de no `IDispatch` admite argumentos con nombre.|  
|DISP_E_OVERFLOW|Uno de los argumentos de `rgvarg` no se pudo convertir en el tipo especificado.|  
